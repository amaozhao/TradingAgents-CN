# 提示词工程最佳实践

**分类**: 提示词工程  
**难度**: 进阶  
**阅读时间**: 12分钟  
**更新日期**: 2026-06-05

---

## 📋 引言

掌握提示词基础后，如何写出高质量的提示词？本文总结了提示词工程的最佳实践，帮助你充分发挥大语言模型的能力。

---

## 🎯 核心原则

### 1. 清晰性原则（Clarity）

**原则**：提示词要清晰明确，避免歧义。

❌ **不清晰**：
```
分析这个
```

✅ **清晰**：
```
请分析股票代码000001（平安银行）2024年第三季度的财务报表，
重点关注营收增长率、净利润率和ROE指标。
```

### 2. 具体性原则（Specificity）

**原则**：提供具体的细节和要求。

❌ **笼统**：
```
告诉我该买什么
```

✅ **具体**：
```
我是一位风险偏好中等的长期投资者，
请基于平安银行（000001）的基本面分析，
说明它是否适合作为进一步研究对象，并列出需要复核的数据。
```

### 3. 结构化原则（Structure）

**原则**：使用清晰的结构组织提示词。

✅ **结构化示例**：
```
# 角色
你是一位专注于银行股的资深分析师

# 任务
分析平安银行（000001）的研究价值

# 数据
- 股价：12.50元
- 市盈率：5.2
- ROE：10.5%

# 要求
1. 分析估值水平
2. 评估盈利能力
3. 给出综合评价和风险提示

# 输出格式
使用Markdown，包含标题和列表
```

### 4. 迭代优化原则（Iteration）

**原则**：根据输出结果不断优化提示词。

**迭代流程**：
```
初始提示词 → 查看输出 → 发现问题 → 优化提示词 → 再次测试
```

---

## 🛠️ 实用技巧

### 技巧1：使用分隔符

使用分隔符清晰地分隔不同部分。

**示例**：
```
请分析以下股票数据：

---数据开始---
股票代码：000001
股票名称：平安银行
当前股价：12.50元
市盈率：5.2
---数据结束---

请基于以上数据进行分析。
```

**常用分隔符**：
- `---`
- `###`
- `"""` (三引号)
- `<data>...</data>` (XML标签)

### 技巧2：指定输出长度

明确指定回答的长度范围。

**示例**：
```
请用100-150字简要分析平安银行的研究价值。
```

或者：
```
请详细分析平安银行的研究价值，回答长度约500-800字。
```

### 技巧3：要求逐步思考

让AI展示思考过程，提高回答质量。

**示例**：
```
请分析平安银行（000001）是否值得进一步研究。

要求：
1. 先列出分析需要考虑的关键因素
2. 逐一分析每个因素
3. 最后给出综合结论

请展示你的思考过程。
```

### 技巧4：提供正反例

通过正反例说明你想要什么。

**示例**：
```
请分析平安银行的研究价值。

✅ 好的回答示例：
"平安银行市盈率5.2，低于行业平均6.5，估值具有吸引力。
但需注意不良贷款率1.2%，略高于同业..."

❌ 不好的回答示例：
"这只股票很好，建议买入。"

请按照好的示例风格回答。
```

### 技巧5：设定约束条件

明确告诉AI什么不能做。

**示例**：
```
请分析平安银行的研究价值。

约束条件：
- 不要预测具体的股价涨跌
- 不要给出"必涨"、"必跌"等绝对判断
- 必须包含风险提示
- 使用客观、理性的语言
```

### 技巧6：使用模板

为常见任务创建提示词模板。

**股票分析模板**：
```
你是一位{专业领域}分析师。

请分析股票{股票代码}（{股票名称}）的{分析维度}。

当前数据：
{数据列表}

分析要求：
{具体要求}

输出格式：
{格式说明}
```

**使用示例**：
```
你是一位银行股分析师。

请分析股票000001（平安银行）的估值水平和盈利能力。

当前数据：
- 市盈率：5.2
- ROE：10.5%
- 净利润增长率：8.5%

分析要求：
1. 与行业平均对比
2. 与历史数据对比
3. 给出评价（低估/合理/高估）

输出格式：
使用Markdown，包含数据对比表格
```

---

## 📊 针对股票分析的最佳实践

### 1. 基本面分析提示词

**模板**：
```
你是一位专注于基本面分析的投资顾问。

请分析{股票代码}的基本面：

财务数据：
- 市盈率：{PE}
- 市净率：{PB}
- ROE：{ROE}
- 营收增长率：{revenue_growth}
- 净利润增长率：{profit_growth}
- 资产负债率：{debt_ratio}

行业信息：
- 所属行业：{industry}
- 行业地位：{position}

请从以下角度分析：
1. 估值水平（与行业对比）
2. 盈利能力
3. 成长性
4. 财务健康度

最后给出综合评分（1-5星）、风险提示和需要复核的数据。
```

### 2. 技术面分析提示词

**模板**：
```
你是一位技术分析专家。

请分析{股票代码}的技术面：

价格数据：
- 当前价格：{current_price}
- 20日均线：{ma20}
- 60日均线：{ma60}
- 近期高点：{high}
- 近期低点：{low}

技术指标：
- MACD：{macd}
- RSI：{rsi}
- 成交量变化：{volume_change}

请分析：
1. 趋势判断（上升/下降/震荡）
2. 支撑位和压力位
3. 技术指标信号
4. 短期信号和不确定性

注意：技术分析仅供参考，不构成投资建议。
```

### 3. 多股对比提示词

**模板**：
```
请对比分析以下{N}只{行业}股票：

{股票列表及数据}

对比维度：
1. 估值水平（PE、PB）
2. 盈利能力（ROE、净利率）
3. 成长性（营收增长、利润增长）
4. 财务健康度（负债率、现金流）

请以表格形式展示对比结果，
并说明各自的优势和劣势，
最后说明不同股票更适合哪些研究假设，并列出主要风险。
```

---

## ⚠️ 常见陷阱

### 陷阱1：过度依赖AI

**问题**：
```
请告诉我明天应该买哪只股票
```

**为什么不好**：
- AI无法预测市场
- 投资决策需要综合考虑个人情况
- 可能导致盲目跟从

**正确做法**：
```
请分析以下3只股票的基本面，
帮助我了解它们的研究价值，
我会结合自己的风险偏好做出决策。
```

### 陷阱2：信息过载

**问题**：
```
请分析这只股票的基本面技术面行业地位竞争优势财务状况
历史表现未来前景管理层质量公司治理股东结构分红政策...
```

**为什么不好**：
- 一次问太多问题
- 回答可能不够深入
- 难以聚焦重点

**正确做法**：
分多次提问，每次聚焦一个主题。

### 陷阱3：缺乏验证

**问题**：直接相信AI的所有输出

**正确做法**：
```
请分析平安银行的财务数据，并注明数据来源。
我会自行验证这些数据的准确性。
```

---

## 💡 高级技巧

### 技巧1：链式提示（Chain of Prompts）

将复杂任务分解为多个步骤。

**示例**：
```
第一步提示：
"请列出分析银行股需要关注的10个关键指标"

第二步提示：
"基于你列出的指标，请分析平安银行的数据：
{数据}"

第三步提示：
"基于以上分析，请给出综合评价、风险提示和需要人工复核的点"
```

### 技巧2：角色扮演

让AI扮演不同角色，获得多角度分析。

**示例**：
```
场景1：你是一位保守的价值投资者
请从价值投资的角度分析平安银行的研究价值

场景2：你是一位激进的成长投资者
请从成长投资的角度分析平安银行的研究价值

场景3：你是一位风险管理专家
请指出投资平安银行的主要风险
```

### 技巧3：自我批评

让AI检查自己的回答。

**示例**：
```
第一步：请分析平安银行的研究价值

第二步：请检查你刚才的分析，指出可能存在的偏见或不足之处

第三步：基于自我批评，给出更全面的分析
```

---

## 📚 总结

### 核心要点

1. **清晰、具体、结构化**是好提示词的基础
2. **使用分隔符、模板、约束条件**提高效率
3. **针对股票分析**有专门的提示词模式
4. **避免陷阱**：过度依赖、信息过载、缺乏验证
5. **高级技巧**：链式提示、角色扮演、自我批评

### 实践建议

1. **建立自己的提示词库**：保存常用的提示词模板
2. **持续优化**：根据效果不断改进提示词
3. **分享交流**：与其他用户交流提示词经验
4. **保持批判性思维**：验证AI的输出

---

## 🔗 相关资源

- 📖 [提示词基础](./prompt-basics.md)

