...
export const configApi = {
  ...
  // 迁移环境变量到厂家管理
  migrateEnvToProviders(): Promise<{ message: string; data: unknown }> {
    return unwrapResponse(ApiClient.post<{ message: string; data: unknown }>('/api/config/llm/providers/migrate-env'))
  },
  ...
  // 测试厂家API
  testProviderAPI(providerId: string): Promise<{ success: boolean; message: string; data?: unknown }> {
    return unwrapResponse(ApiClient.post<{ success: boolean; message: string; data?: unknown }>(`/api/config/llm/providers/${providerId}/test`))
  },
  ...
  getSystemSettings(): Promise<Record<string, unknown>> {
    return unwrapResponse(ApiClient.get<Record<string, unknown>>('/api/config/settings'))
  },

  // 获取默认模型配置
  getDefaultModels(): Promise<{ quick_analysis_model: string; deep_analysis_model: string }> {
    return unwrapResponse(ApiClient.get<Record<string, unknown>>('/api/config/settings')).then(settings => ({
      quick_analysis_model:
        typeof settings.quick_analysis_model === 'string' ? settings.quick_analysis_model : 'qwen-turbo',
      deep_analysis_model:
        typeof settings.deep_analysis_model === 'string' ? settings.deep_analysis_model : 'qwen-max'
    }))
  },

  // 更新系统设置
  updateSystemSettings(settings: Record<string, unknown>): Promise<{ message: string }> {
  ...
  // 导出配置
  exportConfig(): Promise<{ message: string; data: unknown; exported_at: string }> {
    return unwrapResponse(ApiClient.post<{ message: string; data: unknown; exported_at: string }>('/api/config/export'))
  },

  // 导入配置
  importConfig(configData: Record<string, unknown>): Promise<{ message: string }> {
