    ...
    try:
        ...
        if "volume" in standardized_data.columns:
            ...
            if original_vol_sum == mapped_volume_sum:
                print(
                    f"✅ 数据映射正确: 原始vol总和={original_vol_sum}, 映射后volume总和={mapped_volume_sum}"
                )
                return True
            else:
                print(
                    f"❌ 数据映射错误: 原始vol总和={original_vol_sum}, 映射后volume总和={mapped_volume_sum}"
                )
                return False
        else:
...
def test_column_mapping_logic():
    ...
    try:
        TushareAdapter = getattr(
            importlib.import_module("trader.flows.adapter"), "TushareAdapter"
        )
