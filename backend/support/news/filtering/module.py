...
def test_basic_news_filter():
    ...
    try:
        create_news_filter = getattr(
            importlib.import_module("trader.utils.news.filter"), "create_news_filter"
        )
...
def test_enhanced_news_filter():
    ...
    try:
        ...
        # 执行增强过滤
        start_time = time.time()
        enhanced_filtered = enhanced_filter.filter_news_enhanced(
            test_news, min_score=40
        )
        filter_time = time.time() - start_time
...
def test_real_news_filtering():
    ...
    try:
        ...
        print(
            f"  - 过滤率: {(len(real_news) - len(filtered_real_news)) / len(real_news) * 100:.1f}%"
        )
...
def test_news_filter_integration():
    ...
    try:
        apply_news_filtering_patches = getattr(
            importlib.import_module("trader.utils.news.integration"),
