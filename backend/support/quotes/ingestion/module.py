...
from app.core.database import close_db, get_postgres_db, init_db
...
async def test_normalize_stock_code():
    ...
    for input_code, expected, description in test_cases:
        ...
        print(
            f"{status} {description:20s} | 输入: {input_code:12s} | 期望: {expected:8s} | 实际: {result:8s}"
        )
...
async def test_market_quotes_status():
    ...
    if count > 0:
        ...
        for i, doc in enumerate(sample_docs, 1):
            ...
            print(
                f"   {i}. 代码: {code}, 收盘价: {close}, 交易日: {trade_date}, 更新时间: {updated_at}"
            )
        ...
        if abnormal_docs:
            ...
            for doc in abnormal_docs:
                print(
                    f"      - 代码: {doc.get('code')}, 长度: {doc.get('code_length')}"
                )
        else:
            print("   ✅ 所有代码都是标准的6位格式")
...
async def test_historical_data_import():
    ...
    # 获取最新交易日
    latest_doc = (
        await db.stock_daily_quotes.find({"period": "daily"})
        .sort("trade_date", -1)
        .limit(1)
        .to_list(length=1)
    )
...
async def test_akshare_realtime_quotes():
    ...
    try:
        AKShareAdapter = getattr(
            importlib.import_module("app.services.sources.akshare"), "AKShareAdapter"
        )
