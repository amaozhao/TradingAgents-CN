    ...
    try:
        ...
        try:
            news = get_google_news(
                query="Apple AAPL stock", curr_date="2025-06-27", look_back_days=7
            )
...
def test_reddit_tools():
    ...
    try:
        ...
        get_reddit_global_news = getattr(
            importlib.import_module("trader.flows.interface"), "get_reddit_global_news"
        )
        get_reddit_company_news = getattr(
            importlib.import_module("trader.flows.interface"), "get_reddit_company_news"
        )

        print("✅ Reddit工具函数导入成功")

        # 检查Reddit数据目录
        reddit_data_dir = Path("backend/trader/flows/data/cache/reddit_data")
            ...
            if subdirs:
                ...
                try:
                    ...
                    global_news = get_reddit_global_news(
                        start_date="2025-06-27", look_back_days=1, max_limit_per_day=5
                    )
