    ...
    for depth_name, days_back, description in test_cases:
        ...
        try:
            ...
            results[depth_name] = {
                ...
                "fundamentals_length": len(fundamentals_data)
...
def compare_performance():
    ...
    if new_result["success"]:
        ...
        if successful_old:
            ...
            print(
                f"   - 数据减少: {(avg_old_length - new_result['data_length']) / avg_old_length * 100:.1f}%"
            )
            ...
            print(
                f"   - 时间节省: {(avg_old_time - new_result['processing_time']) / avg_old_time * 100:.1f}%"
            )

            # 详细对比表
            print("\n📋 详细对比表:")
            print(
                f"{'策略':<25} | {'天数':<4} | {'数据量(字符)':<12} | {'时间(秒)':<8} | {'状态'}"
            )
            print("-" * 70)
            ...
            for depth, result in old_results.items():
                ...
                proc_time = (
                    f"{result['processing_time']:.2f}" if result["success"] else "N/A"
                )
                print(
            ...
            print(
                f"{'优化后策略':<25} | {new_result['days_requested']:<4} | {data_len:<12} | {proc_time:<8} | ✅"
            )
            ...
            print(
                f"   ✅ 处理时间平均节省 {(avg_old_time - new_result['processing_time']) / avg_old_time * 100:.1f}%"
            )
