...
def test_google_news_import():
    ...
    try:
        ...
        googlenews_utils = getattr(
            importlib.import_module("trader.flows"), "googlenewsutils"
        )
        ...
        if hasattr(googlenews_utils, "get_news_data"):
            print("✅ get_news_data函数存在")
        else:
            print("❌ get_news_data函数不存在")
...
def check_actual_file_contents():
    ...
    # 检查Google News文件
    try:
        google_file = Path("backend/trader/flows/googlenewsutils.py")
    ...
    # 检查Reddit文件
    try:
        reddit_file = Path("backend/trader/flows/dataflowreddit_utils.py")
