...
from trader.flows.china import OptimizedChinaDataFlow
...
def test_financial_data_validation():
    ...
    result = dataflow._parse_postgres_financial_data(normal_data, 10.0)
    ...
    result = dataflow._parse_postgres_financial_data(abnormal_data, 10.0)
    print(f"ROE: {result.get('roe')}")
    print(f"ROA: {result.get('roa')}")
    ...
    result = dataflow._parse_postgres_financial_data(boundary_data, 10.0)
    print(f"ROE: {result.get('roe')}")
    print(f"ROA: {result.get('roa')}")
    print(f"毛利率: {result.get('gross_margin')}")
    print(f"净利率: {result.get('net_margin')}")
    ...
    result = dataflow._parse_postgres_financial_data(out_of_range_data, 10.0)
