...
"""
        ...
        cache_key = cache.save_fundamentals_data(
            test_symbol, test_data, data_source="test"
        )
...
def test_fundamentals_with_cache():
    ...
    try:
        ...
        if second_time < first_time:
            print(
                f"✅ 缓存生效！第二次获取速度提升了 {((first_time - second_time) / first_time * 100):.1f}%"
            )
        else:
    ...
    try:
        ...
        for cache_type, config in cache.cache_config.items():
            if "fundamentals" in cache_type:
                print(
                    f"  - {cache_type}: TTL={config['ttl_hours']}小时, 描述={config['description']}"
                )
        ...
        china_key = cache.find_cached_fundamentals_data(
            china_symbol, data_source="test"
        )
        if china_key:
