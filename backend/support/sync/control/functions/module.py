...
async def test_sync_control_functions():
    ...
    try:
        ...
        try:
            # 先检查是否有缓存数据
            cache_count_before = await db.sync_status.count_documents(
            ...
            cache_count_after = await db.sync_status.count_documents(
                {"job": "stock_basics_multi_source"}
            )
        ...
        try:
            ...
            if not available_adapters:
                ...
                print(
                    f"   📡 可用数据源: {[adapter.name for adapter in available_adapters]}"
                )
        ...
        try:
            ...
            if final_status.get("data_sources_used"):
                print(
                    f"   🔗 使用的数据源: {', '.join(final_status.get('data_sources_used'))}"
                )
...
async def test_api_endpoints():
    ...
    try:
        # 这里可以添加HTTP客户端测试
        # 但为了简化，我们直接调用路由函数
        get_sync_status = getattr(
            importlib.import_module("app.routers.sources"), "get_sync_status"
        )
        clear_sync_cache = getattr(
            importlib.import_module("app.routers.sources"), "clear_sync_cache"
        )
        run_stock_basics_sync = getattr(
            importlib.import_module("app.routers.sources"), "run_stock_basics_sync"
        )
