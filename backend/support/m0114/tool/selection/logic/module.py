    ...
    try:
        ...
        for test_config in test_configs:
            ...
            try:
                ...
                print(
                    f"   主要市场工具: {market_tool_names[1] if len(market_tool_names) > 1 else 'N/A'}"
                )
                print(
                    f"   主要新闻工具: {news_tool_names[0] if news_tool_names else 'N/A'}"
                )
                print(
                    f"   主要社交工具: {social_tool_names[0] if social_tool_names else 'N/A'}"
                )
    ...
    for i, openai_config in enumerate(openai_scenarios, 1):
        ...
        try:
            ...
            for data_config in data_configs:
                os.environ["REALTIME_DATA_ENABLED"] = data_config[
                ...
                print(
                    f"     REALTIME_DATA_ENABLED={data_config['REALTIME_DATA_ENABLED']} "
                    f"-> {expected_mode} ✅"
                )
