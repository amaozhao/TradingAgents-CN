...
def test_social_media_analyst_tools():
    ...
    try:
        ...
        social_methods = [
            ...
            if any(
                keyword in m.lower()
                for keyword in ["social", "reddit", "twitter", "sentiment"]
            )
        ]
        ...
        # 检查是否包含在线工具
        tool_names = [
            GoogleToolCallHandler._get_tool_name(tool) for tool in social_tools
        ]
        ...
        for tool_name in tool_names:
            if (
                "twitter" in tool_name.lower()
                or "reddit" in tool_name.lower()
                and "online" in tool_name.lower()
            ):
...
def test_google_tool_handler_improvements():
    ...
    for i, tool_call in enumerate(mock_tool_calls):
        ...
        if GoogleToolCallHandler._validate_tool_call(tool_call, i, "测试分析师"):
            ...
            print("  ⚠️ 验证失败，尝试修复...")
            validated_call = GoogleToolCallHandler._fix_tool_call(
                tool_call, i, "测试分析师"
            )
            if validated_call:
    ...
    return {
        ...
        "improvement_ratio": (len(mock_tool_calls) - len(valid_tool_calls))
        / len(mock_tool_calls),
    }
...
def main():
    ...
    try:
        ...
        print("\n📋 测试结果总结:")
        print(
            f"1. ✅ OpenAI API状态: {'禁用' if not config_status['openai_enabled'] else '启用'}"
        )
        print(
            f"2. ✅ 在线工具状态: {'禁用' if not config_status['online_tools'] else '启用'}"
        )
