...
def test_sanitize_real_export_file():
    ...
    # 读取真实导出文件
    with open(
        "backend/install/database_export_config_2025-10-25.json", "r", encoding="utf-8"
    ) as f:
    ...
    for config in sanitized_data.get("system_configs", []):
        for llm_config in config.get("llm_configs", []):
            assert llm_config.get("api_key") == "", (
                f"api_key 应该被清空，但实际值为: {llm_config.get('api_key')}"
            )
        ...
        for key in ["finnhub_api_key", "tushare_token", "reddit_client_secret"]:
            if key in system_settings:
                assert system_settings[key] == "", (
                    f"{key} 应该被清空，但实际值为: {system_settings[key]}"
                )
    ...
    with open(
        "backend/install/database_export_config_2025-10-25_SANITIZED.json",
        "w",
        encoding="utf-8",
    ) as f:
    ...
    print(
        "✅ 脱敏后的文件已保存到: install/database_export_config_2025-10-25_SANITIZED.json"
    )
