...
def test_data_consistency_checker():
    ...
    try:
        ...
        getattr(
            importlib.import_module("app.services.consistency"),
        ...
        # 使用一致性检查获取数据
        data, source, consistency_report = (
            manager.get_daily_basic_with_consistency_check(trade_date)
        )
        ...
        if data is not None and not data.empty:
            ...
            if consistency_report:
                print("\n📊 一致性检查报告:")
                print(
                    f"   数据一致性: {'✅ 一致' if consistency_report['is_consistent'] else '❌ 不一致'}"
                )
                ...
                if consistency_report["differences"]:
                    ...
                    for metric, diff_info in consistency_report["differences"].items():
                        if (
                            isinstance(diff_info, dict)
                            and "difference_pct" in diff_info
                        ):
                            print(f"   {metric}:")
                            print(
                                f"     主数据源值: {diff_info.get('primary_value', 'N/A')}"
                            )
                            print(
                                f"     次数据源值: {diff_info.get('secondary_value', 'N/A')}"
                            )
                            if diff_info.get("difference_pct") is not None:
                                print(
                                    f"     差异百分比: {diff_info['difference_pct']:.2%}"
                                )
                            print(
                                f"     是否显著: {'是' if diff_info.get('is_significant') else '否'}"
                            )
        ...
        if manager.consistency_checker and len(available_adapters) >= 2:
            ...
            if primary_data is not None and secondary_data is not None:
                consistency_result = (
                    manager.consistency_checker.check_daily_basic_consistency(
                        primary_data,
                        secondary_data,
                ...
                # 测试冲突解决
                final_data, strategy = (
                    manager.consistency_checker.resolve_data_conflicts(
                        primary_data, secondary_data, consistency_result
                    )
                )
                print(f"   解决策略: {strategy}")
                print(
                    f"   最终数据条数: {len(final_data) if final_data is not None else 0}"
                )
            else:
...
def test_mock_data_consistency():
    ...
    try:
        DataConsistencyChecker = getattr(
            importlib.import_module("app.services.consistency"),
            "DataConsistencyChecker",
        )
