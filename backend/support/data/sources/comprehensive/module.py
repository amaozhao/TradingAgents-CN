...
def test_china_stock_data_sources():
    ...
    for symbol in test_symbols:
        ...
        try:
            print("🔍 测试优化版本...")
            get_china_stock_data_cached = getattr(
                importlib.import_module("trader.flows.china"),
                "get_china_stock_data_cached",
            )

            start_time = time.time()
            result = get_china_stock_data_cached(
...
def test_us_stock_data_sources():
    ...
    for symbol in test_symbols:
        ...
        try:
            ...
            start_time = time.time()
            result = get_us_stock_data_cached(
                symbol, start_date, end_date, force_refresh=True
            )
            end_time = time.time()
...
def test_news_data_sources():
    ...
    for symbol in test_symbols:
        ...
        try:
            print("🔍 测试实时新闻聚合器...")
            RealtimeNewsAggregator = getattr(
                importlib.import_module("trader.flows.real.time"),
        ...
        try:
            ...
            get_finnhub_news = getattr(
                importlib.import_module("trader.flows.interface"), "get_finnhub_news"
            )
            ...
            for symbol, symbol_results in category_results.items():
                ...
                for test_type, result in symbol_results.items():
                    ...
                    if result.get("success"):
                        ...
                        print(
                            f"      ✅ {test_type}: {time_taken:.2f}s, {data_length}字符"
                        )
                    else:
                ...
                for test_type, result in symbol_results.items():
                    if result.get("success") and "time" in result:
                        ...
                        fastest_times.append(
                            (f"{category}-{symbol}-{test_type}", time_taken)
                        )
                        slowest_times.append(
                            (f"{category}-{symbol}-{test_type}", time_taken)
                        )
...
def print_recommendations(all_results: Dict):
    ...
    china_success_rate = (
        (china_success_count / china_total_count * 100) if china_total_count > 0 else 0
    )
    ...
    us_success_rate = (
        (us_success_count / us_total_count * 100) if us_total_count > 0 else 0
    )
