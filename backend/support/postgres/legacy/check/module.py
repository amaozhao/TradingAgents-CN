...
try:
    ...
    print("✅ PostgreSQL报告管理器导入成功")
except ImportError as e:
    raise RuntimeError(f"PostgreSQL报告管理器导入失败: {e}") from e


def check_postgres_connection():
    """检查PostgreSQL连接状态"""
    print("\n🔍 检查PostgreSQL连接状态...")
    print(f"连接状态: {postgres_report_manager.connected}")
...
def check_analysis_records():
    ...
    try:
        # 获取所有记录
        all_reports = postgres_report_manager.get_all_reports(limit=50)
        print(f"总记录数: {len(all_reports)}")

        if not all_reports:
            print("⚠️ PostgreSQL中没有分析记录")
            return
            ...
            if reports:
                ...
                for module_name, content in reports.items():
                    ...
                    if content_length == 0:
                        ...
                        "暂无详细分析" in content or "演示数据" in content
                    ):
                        print("      ⚠️ 内容为演示数据或占位符")
                    else:
                        print("      ✅ 内容正常")
...
def check_specific_stock(stock_symbol="000001"):
    ...
    try:
        reports = postgres_report_manager.get_analysis_reports(
            limit=10, stock_symbol=stock_symbol
        )
