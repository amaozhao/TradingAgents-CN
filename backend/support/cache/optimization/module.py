    ...
    try:
        ...
        if hasattr(cache, "cache_config"):
            ...
            for config_name, config_data in cache.cache_config.items():
                print(
                    f"  - {config_name}: TTL={config_data.get('ttl_hours')}h, 描述={config_data.get('description')}"
                )
...
def test_china_stock_cache():
    ...
    try:
        get_optimized_china_data_provider = getattr(
    ...
    try:
        ...
        if us_cache_key:
            is_valid = cache.is_cache_valid(
                us_cache_key, symbol="AAPL", data_type="stock_data"
            )
        ...
        if china_cache_key:
            is_valid = cache.is_cache_valid(
                china_cache_key, symbol="000001", data_type="stock_data"
            )
