...
def test_optimized_fundamentals():
    ...
    for depth_name, days_back, description in test_cases:
        ...
        try:
            ...
            result = toolkit.get_stock_fundamentals_unified.__func__(
                test_symbol, start_date, end_date, end_date
            )
            ...
            if result:
                ...
                has_fundamentals = (
                    "财务" in result or "基本面" in result or "投资建议" in result
                )
    ...
    print(
        f"🎯 成功率: {successful_tests}/{total_tests} ({successful_tests / total_tests * 100:.1f}%)"
    )
    ...
    for depth_name, result in results.items():
        ...
        print(
            f"   {result['description']:20} | {status} | {result['data_length']:6,} 字符"
        )
