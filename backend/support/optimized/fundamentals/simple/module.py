...
def test_optimized_fundamentals_logic():
    ...
    try:
        ...
        print(
            f"   - 价格数据占比: {len(current_price_data) / len(final_result) * 100:.1f}%"
        )
        print(
            f"   - 基本面数据占比: {len(fundamentals_data) / len(final_result) * 100:.1f}%"
        )

        # 检查数据质量
        has_price = (
            "价格" in final_result or "股价" in final_result or "Price" in final_result
        )
        has_fundamentals = (
            "财务" in final_result
            or "基本面" in final_result
            or "投资建议" in final_result
        )
