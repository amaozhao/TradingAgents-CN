...
def test_optimized_data_depth():
    ...
    for i, depth in enumerate(depth_levels):
        ...
        try:
            ...
            historical_data_mentions = sum(
                1
                for line in lines
                if "历史" in line or "天数据" in line or "days" in line.lower()
            )
    ...
    if len(valid_results) >= 2:
        ...
        # 1. 检查是否还有历史数据相关内容
        total_historical_mentions = sum(
            r["historical_mentions"] for r in valid_results.values()
        )
        ...
        if level_1_modules < level_3_modules <= level_5_modules:
            ...
            print(
                f"   ⚠️  数据深度级别可能需要调整: L1={level_1_modules}, L3={level_3_modules}, L5={level_5_modules}"
            )
