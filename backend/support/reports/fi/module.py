...
def test_reports_and_analysts_fix():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"📋 选择的分析师: {analysis_request['parameters']['selected_analysts']}"
            )
        else:
        ...
        # 5. 检查 PostgreSQL document store 保存的数据
        print("\n5. 检查 PostgreSQL document store 保存的数据...")
        ...
        try:
            ...
            # 查找最新的记录
            latest_record = (
                collection.find({"stock_symbol": "000006"})
                .sort("created_at", -1)
                .limit(1)
            )

            for record in latest_record:
                print("📋 PostgreSQL document store 记录详情:")
                ...
                if reports:
                    print(f"✅ PostgreSQL document store 包含 {len(reports)} 个报告:")
                    for report_type, content in reports.items():
                        if isinstance(content, str):
                            print(f"   - {report_type}: {len(content)} 字符")
                            # 显示报告内容的前100个字符作为预览
                    ...
                    print("❌ PostgreSQL document store 未包含reports字段或为空")
                    return False

            print("❌ 未找到 PostgreSQL document store 记录")
            return False

        except Exception as e:
            print(f"❌ PostgreSQL document store 检查失败: {e}")
            return False
        finally:
            if "client" in locals():
                client.close()
