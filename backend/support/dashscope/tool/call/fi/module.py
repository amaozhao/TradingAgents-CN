...
数据来源：Google News"""
    ...
    if "DashScope" in llm.__class__.__name__:
        if tool_call_count == 0:
            ...
            try:
                ...
                forced_news = toolkit.get_realtime_stock_news().invoke(
                ...
                if forced_news and len(forced_news.strip()) > 100:
                    ...
                    print(
                        f"✅ 强制补救成功，生成基于真实数据的报告，长度: {len(report)} 字符"
                    )
    ...
    print(
        f"   是否包含真实新闻: {'是' if '东方财富新闻' in report or 'Google新闻' in report else '否'}"
    )
    print(
        f"   补救机制状态: {'成功' if len(report) > len(result.content) else '未触发或失败'}"
    )
