...
def test_data_source_connection():
    ...
    try:
        ...
        print(
            f"🔗 AKShare连接状态: {'✅ 已连接' if akshare_provider.connected else '❌ 未连接'}"
        )
    ...
    try:
        ...
        print(
            f"🔗 Tushare连接状态: {'✅ 已连接' if tushare_provider.connected else '❌ 未连接'}"
        )
    except Exception as e:
        ...
        for symbol in test_symbols:
            ...
            try:
                ...
                if financial_data:
                    ...
                    if main_indicators:
                        pe = main_indicators.get(
                            "市盈率", main_indicators.get("PE", "N/A")
                        )
                        pb = main_indicators.get(
                            "市净率", main_indicators.get("PB", "N/A")
                        )
                        roe = main_indicators.get(
                            "净资产收益率", main_indicators.get("ROE", "N/A")
                        )
...
def test_financial_metrics_with_data_source():
    ...
    for symbol in test_symbols:
        ...
        try:
            ...
            if pe_line:
                pe_value = (
                    pe_line.split("**")[2].strip()
                    if "**" in pe_line
                    else pe_line.split(":")[1].strip()
                )
                print(f"📈 PE: {pe_value}")

            if pb_line:
                pb_value = (
                    pb_line.split("**")[2].strip()
                    if "**" in pb_line
                    else pb_line.split(":")[1].strip()
                )
                print(f"📈 PB: {pb_value}")

            if roe_line:
                roe_value = (
                    roe_line.split("**")[2].strip()
                    if "**" in roe_line
                    else roe_line.split(":")[1].strip()
                )
