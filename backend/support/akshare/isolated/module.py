...
def test_akshare_utils_direct():
    ...
    try:
        ...
        akshare_utils_path = os.path.join(
            project_root, "trader", "dataflows", "akshare_utils.py"
        )
...
def check_data_source_enum():
    ...
    try:
        ...
        data_source_manager_path = os.path.join(
            project_root, "trader", "dataflows", "data_source_manager.py"
        )
...
def main():
    ...
    # 分析结果
    if test_results.get("akshare_direct", False) and test_results.get(
        "akshare_utils_direct", False
    ):
        print("\n🎉 AKShare核心功能完全正常！")
