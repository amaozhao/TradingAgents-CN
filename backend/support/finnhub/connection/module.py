    ...
    try:
        ...
        try:
            fundamentals_result = toolkit.get_fundamentals_openai.invoke(
                {"ticker": "AAPL", "curr_date": "2025-06-29"}
            )
            print("✅ OpenAI基本面API调用成功")
            print(
                f"基本面数据长度: {len(fundamentals_result) if fundamentals_result else 0}"
            )
...
def test_china_stock_api():
    ...
    try:
        ...
        try:
            ...
            print(
                f"基本面数据长度: {len(china_fundamentals_result) if china_fundamentals_result else 0}"
            )
