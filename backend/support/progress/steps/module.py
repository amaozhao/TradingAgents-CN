    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(
            ...
            if response.status == 200:
                ...
                print(
                    f"📋 登录响应: {json.dumps(result, indent=2, ensure_ascii=False)}"
                )
...
async def start_analysis(token: str) -> str:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            f"{BASE_URL}/api/analysis/single", headers=headers, json=analysis_data
        ) as response:
            if response.status == 200:
                result = await response.json()
                print(
                    f"📋 分析响应: {json.dumps(result, indent=2, ensure_ascii=False)}"
                )
...
async def monitor_task_progress(
    token: str, task_id: str, max_iterations: int = 60, interval: int = 3
):
    """监控任务进度"""
    print("\n🔄 开始监控任务进度...")
