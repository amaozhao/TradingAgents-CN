...
from app.services.memory import TaskState, TaskStatus
...
def test_progress_time_calculation_basic():
    ...
    print(
        f"   预计总时长: {data['estimated_total_time']:.1f}秒 ({data['estimated_total_time'] / 60:.1f}分钟)"
    )
    print(
        f"   预计剩余: {data['remaining_time']:.1f}秒 ({data['remaining_time'] / 60:.1f}分钟)"
    )
