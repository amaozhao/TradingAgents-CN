...
async def test_init_database_initializes_postgres_when_runtime_enabled(monkeypatch):
    ...
    async def fake_init_postgres_document_store():
        database.db_manager.postgres_client = object()
        database.db_manager.postgres_db = object()
    ...
    monkeypatch.setattr(
        database.db_manager, "init_postgres", fake_init_postgres_document_store
    )
    monkeypatch.setattr(database.db_manager, "init_redis", fake_init_redis)
    monkeypatch.setattr(
        database, "init_postgres_if_enabled", fake_init_postgres_if_enabled
    )
    monkeypatch.setattr(database, "init_database_views_and_indexes", fake_init_views)

    await database.init_database()

    assert calls == ["postgres", "redis", "postgres", "views"]
...
async def test_close_database_closes_postgres(monkeypatch):
    ...
    async def fake_close_connections():
        calls.append("postgres_redis")

    async def fake_close_postgres_if_enabled():
        calls.append("postgres")

    monkeypatch.setattr(
        database.db_manager, "close_connections", fake_close_connections
    )
    monkeypatch.setattr(
        database, "close_postgres_if_enabled", fake_close_postgres_if_enabled
    )

    await database.close_database()

    assert calls == ["postgres_redis", "postgres"]
