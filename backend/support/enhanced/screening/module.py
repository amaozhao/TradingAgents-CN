...
async def test_enhanced_screening():
    ...
    try:
        ...
        get_enhanced_screening_service = getattr(
        ...
        for field in fields[:5]:  # 显示前5个字段
            print(
                f"  - {field['name']}: {field['display_name']} ({field['field_type']})"
            )
        ...
        result = await service.screen_stocks(
            conditions=conditions, limit=10, use_database_optimization=True
        )
        time.time()
        ...
        # 简单条件（适合数据库优化）
        simple_conditions = [
            ScreeningCondition(field="total_mv", operator=OperatorType.GTE, value=50)
        ]

        # 数据库优化方式
        start_time = time.time()
        db_result = await service.screen_stocks(
            conditions=simple_conditions, limit=20, use_database_optimization=True
        )
        ...
        print(
            f"  - 数据库优化: {db_result.get('took_ms', 0)}ms, 结果数: {len(db_result['items'])}"
        )
        print(
            f"  - 传统方式: {traditional_result.get('took_ms', 0)}ms, 结果数: {len(traditional_result['items'])}"
        )
        print(
            f"  - 性能提升: {traditional_time / db_time:.1f}x"
            if db_time > 0
            else "  - 无法计算性能提升"
        )
