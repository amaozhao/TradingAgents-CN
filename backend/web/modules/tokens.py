...
from trader.config.manager import UsageRecord, config_manager
...
def render_tokens():
    ...
    with st.sidebar:
        ...
        time_range = st.selectbox(
            "统计时间范围", ["今天", "最近7天", "最近30天", "最近90天", "全部"], index=2
        )
...
def render_overview_metrics(stats: Dict[str, Any], time_range: str):
    ...
    with col2:
        st.metric(
            label="🔢 总调用次数", value=f"{stats['total_requests']:,}", delta=None
        )
...
def render_provider_statistics(stats: Dict[str, Any]):
        ...
        [
            {
                ...
                "平均成本(¥)": f"{data['cost'] / data['requests']:.4f}"
                if data["requests"] > 0
                else "0.0000",
            }
    ...
    with col1:
        # 成本对比柱状图
        cost_data = {
            provider: data["cost"] for provider, data in provider_stats.items()
        }
        fig_bar = px.bar(
    ...
    with col2:
        # 调用次数对比
        requests_data = {
            provider: data["requests"] for provider, data in provider_stats.items()
        }
        fig_requests = px.bar(
...
def render_cost_trends(records: List[UsageRecord]):
    ...
    # 按日期聚合
    daily_stats = (
        df_records.groupby("date").agg({"cost": "sum", "tokens": "sum"}).reset_index()
    )

    # 创建双轴图表
    fig = make_subplots(
        specs=[[{"secondary_y": True}]], subplot_titles=["每日成本和Token使用趋势"]
    )
...
def render_detailed_records_table(records: List[UsageRecord]):
    ...
    records_df = pd.DataFrame(
        [
            {
                "时间": datetime.fromisoformat(record.timestamp).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
                "供应商": record.provider,
                ...
                "成本(¥)": f"{record.cost:.4f}",
                "会话ID": record.session_id[:12] + "..."
                if len(record.session_id) > 12
    ...
    if total_pages > 1:
        page = st.selectbox(
            f"页面 (共{total_pages}页, {total_records}条记录)",
...
def main():
    """主函数"""
    st.set_page_config(
        page_title="Token统计 - TradingAgents", page_icon="💰", layout="wide"
    )
