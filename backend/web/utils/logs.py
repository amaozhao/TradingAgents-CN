...
if TYPE_CHECKING:
    from web.utils.progress import AsyncProgressTracker
    ...
    def emit(self, record):
        ...
        try:
            ...
            if "[模块开始]" in message or "[模块完成]" in message:
                # 尝试从消息中提取股票代码来匹配分析
                self._extract_stock_symbol(message)
                ...
                for analysis_id, tracker in trackers_copy.items():
                    ...
                    if (
                        ...
                        try:
                            ...
                            print(
                                f"📊 [进度集成] 转发消息到 {analysis_id}: {message[:50]}..."
                            )
                            break  # 只更新第一个匹配的跟踪器
                        except Exception as e:
                            print(f"❌ [进度集成] 更新失败: {e}")
...
def register_analysis_tracker(analysis_id: str, tracker):
    """注册分析跟踪器"""
    setup_progress_log_integration()
