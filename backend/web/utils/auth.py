...
class AuthManager:
    ...
    def _inject_auth_cache_js(self):
        ...
        window.AuthCache = {
            ...
            saveAuth: function(userInfo) {
                ...
                localStorage.setItem('trading_agents_auth', JSON.stringify(authData));
    ...
    def authenticate(self, username: str, password: str) -> Tuple[bool, Optional[Dict]]:
        ...
        if username not in users:
            ...
            # 记录登录失败
            if activity:
                activity.log_login(username, False, "用户不存在")
            return False, None
        ...
        if password_hash == user_info["password_hash"]:
            ...
            # 记录登录成功
            if activity:
                activity.log_login(username, True)
            return True, {
                "username": username,
                "role": user_info["role"],
            ...
            # 记录登录失败
            if activity:
                activity.log_login(username, False, "密码错误")
            return False, None
    ...
    def logout(self):
        ...
        try {
            localStorage.removeItem('trading_agents_auth');
            localStorage.removeItem('trading_agents_last_activity');
            console.log('✅ 认证数据已清除');
        } catch (e) {
