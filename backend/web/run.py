...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--no-clean":
            ...
            logger.info("=")
            logger.info("用法:")
            logger.info("  python run.py           # 默认启动（清理项目缓存）")
            logger.info("  python run.py --no-clean      # 跳过缓存清理")
            logger.info("  python run.py --force-clean   # 强制清理所有缓存")
            logger.info("  python run.py --help          # 显示帮助")
            logger.info("\n环境变量:")
