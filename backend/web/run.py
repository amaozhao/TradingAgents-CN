...
# 添加后端源码目录到Python路径，并保留仓库根目录用于 .env/data/config
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--no-clean":
            ...
            logger.info(f"=")
            logger.info(f"用法:")
            logger.info(f"  python run.py           # 默认启动（清理项目缓存）")
            logger.info(f"  python run.py --no-clean      # 跳过缓存清理")
            logger.info(f"  python run.py --force-clean   # 强制清理所有缓存")
            logger.info(f"  python run.py --help          # 显示帮助")
            logger.info(f"\n环境变量:")
