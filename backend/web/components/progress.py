...
from web.utils.progress import format_time, get_progress_by_id
                        ...
                        if not st.session_state.get("analysis"):
                            try:
                                format_analysis = getattr(
                                    importlib.import_module("web.utils.analysis"),
                                    "format_analysis",
                                )
    ...
    if status == "failed":
        ...
        if st.button(
            ...
            if not st.session_state.get("analysis"):
                try:
                    format_analysis = getattr(
                        importlib.import_module("web.utils.analysis"), "format_analysis"
                    )
                    raw_results = progress_data.get("raw_results")
                    if raw_results:
                        formatted_results = format_analysis(raw_results)
            ...
            if not st.session_state.get("analysis"):
                try:
                    progress_getter = getattr(
                        importlib.import_module("web.utils.progress"),
                        "get_progress_by_id",
                    )
                    format_analysis = getattr(
                        importlib.import_module("web.utils.analysis"), "format_analysis"
                    )
                    progress_data = progress_getter(analysis_id)
                    if progress_data and progress_data.get("raw_results"):
                        formatted_results = format_analysis(
