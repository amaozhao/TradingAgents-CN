    ...
    with st.form("form", clear_on_submit=False):
            ...
            if market_type == "美股":
                stock_symbol = (
                    st.text_input(
                        ...
                        value=cached_stock
                        if (
                            cached_config and cached_config.get("market_type") == "美股"
                        )
                        else "",
                ...
                stock_symbol = (
                    st.text_input(
                        ...
                        value=cached_stock
                        if (
                            cached_config and cached_config.get("market_type") == "港股"
                        )
                        else "",
        ...
        if market_type_changed:
            if market_type == "A股":
                # 切换到A股：移除社交媒体分析师
                cached_analysts = [
                    analyst for analyst in cached_analysts if analyst != "social"
                ]
        ...
        with st.expander("🔧 高级选项"):
            include_sentiment = st.checkbox(
                "包含情绪分析", value=True, help="是否包含市场情绪和投资者情绪分析"
            )

            include_risk_assessment = st.checkbox(
                "包含风险评估", value=True, help="是否包含详细的风险因素评估"
            )
