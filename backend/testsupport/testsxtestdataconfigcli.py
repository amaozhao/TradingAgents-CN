...
from tradingagents.dataflows.dataflowsconfig import get_data_dir, set_data_dir, initialize_config
...
def test_cli_commands():
    ...
    print("   python -m cli.climain data-config")
    print("   python -m cli.climain data-config --show")
    print()
    print("2. 设置自定义数据目录 | Set custom data directory:")
    print("   python -m cli.climain data-config --set C:\\custom\\trading\\data")
    print()
    print("3. 重置为默认配置 | Reset to default configuration:")
    print("   python -m cli.climain data-config --reset")
    print()
    print("4. 查看所有可用命令 | View all available commands:")
    print("   python -m cli.climain --help")
    print()
