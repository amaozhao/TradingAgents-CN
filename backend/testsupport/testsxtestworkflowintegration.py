...
def test_news_analyst_integration():
    ...
    try:
        # 1. 检查新闻分析师的工具绑定
        print(f"\n📰 第一步：检查新闻分析师的工具绑定...")
        from tradingagents.agents.analysts.newsanalyst import create_news_analyst
        ...
        # 检查统一新闻工具是否能正常导入
        try:
            from tradingagents.tools.unifiednewstool import create_unified_news_tool
        ...
        # 读取新闻分析师源码
        news_analyst_file = "backend/tradingagents/agents/analysts/newsanalyst.py"
        try:
            ...
            # 检查关键集成点
            integration_checks = [
                ("统一新闻工具导入", "from tradingagents.tools.unifiednewstool import create_unified_news_tool"),
        ...
        # 检查工作流程设置文件
        setup_file = "backend/tradingagents/graph/setup.py"
        try:
            with open(setup_file, "r", encoding="utf-8") as f:
                setup_code = f.read()
            
            workflow_checks = [
                ("新闻分析师导入", "from tradingagents.agents.analysts.newsanalyst import create_news_analyst"),
