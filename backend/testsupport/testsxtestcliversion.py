...
def test_cli_imports():
    ...
    try:
        # 测试导入CLI主模块
        from cli.climain import app, console
        print("✅ CLI主模块导入成功")
        ...
        # 测试导入工具函数
        from cli.cliutils import get_user_selections
        print("✅ CLI工具函数导入成功")
...
def test_cli_cost_tracking():
    ...
    try:
        from tradingagents.config.configmanager import config_manager, token_tracker
...
def main():
    ...
    if overall_success:
        print("\n🎉 CLI版本测试全部通过！")
        print("   命令行版本可以正常使用")
        print("   建议运行: python -m cli.climain analyze")
    else:
        print("\n❌ CLI版本测试有失败项")
        print("   请检查失败的测试项")
