...
def test_selections_dictionary_keys():
    ...
    try:
        from cli.climain import get_user_selections
        
        # 模拟用户输入
        with patch('typer.prompt') as mock_prompt, \
             patch('cli.climain.select_market') as mock_market, \
             patch('cli.climain.select_analysts') as mock_analysts, \
             patch('cli.climain.select_research_depth') as mock_depth, \
             patch('cli.climain.select_llm_provider') as mock_llm, \
             patch('cli.climain.select_shallow_thinking_agent') as mock_shallow, \
             patch('cli.climain.select_deep_thinking_agent') as mock_deep, \
             patch('cli.climain.console.print'):
