...
def _resync_reloaded_modules():
    ...
    Several tests below reload ``cli.cliutils`` to re-evaluate OLLAMA_BASE_URL.
    That leaves ``cli.climain``'s star-imported names (e.g. get_ticker) bound to
    ...
    import cli.cliutils
    import cli.climain
    importlib.reload(cli.cliutils)
    importlib.reload(cli.climain)
...
def _reload_client():
    import tradingagents.llmclients.openaiclient as mod
...
# ---- cli.cliutils side: select_llm_provider dropdown -------------------------
...
def test_cli_dropdown_uses_env(monkeypatch):
    ...
    import cli.cliutils as cli_utils
...
def test_ollama_model_labels_no_local_suffix():
    ...
    from tradingagents.llmclients.modelcatalog import get_model_options
