...
# 导入需要测试的模块
from tradingagents.dataflows.realtimenewsutils import get_realtime_stock_news
from tradingagents.dataflows.googlenewsutils import getNewsData, make_request
from tradingagents.dataflows.akshareutils import get_stock_news_em
...
class TestNewsTimeoutFix(unittest.TestCase):
    ...
    def test_news_source_fallback(self):
        """测试新闻源轮询机制"""
        # 模拟实时新闻聚合器失败
        with patch('tradingagents.dataflows.realtimenewsutils.RealtimeNewsAggregator.get_realtime_stock_news') as mock_aggregator:
            ...
            with patch('tradingagents.dataflows.interface.get_google_news') as mock_google_news:
                ...
                # 模拟东方财富新闻获取成功
                with patch('tradingagents.dataflows.akshareutils.get_stock_news_em') as mock_em_news:
