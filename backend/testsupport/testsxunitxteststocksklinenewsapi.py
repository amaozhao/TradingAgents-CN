...
# Build a minimal app that mounts only the stocks router to avoid triggering app.appmain lifespan
...
def test_kline_ok_source_and_adj(client):
    ...
    with patch("app.services.datasources.manager.DataSourceManager.get_kline_with_fallback", return_value=(items, "tushare")):
...
def test_news_ok_with_announcements_and_source(client):
    ...
    with patch("app.services.datasources.manager.DataSourceManager.get_news_with_fallback", return_value=(items, "tushare")):
