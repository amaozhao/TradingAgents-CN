...
使用方法：
    python scripts/sync/financial/data/script.py 600036  # 同步单只股票
...
async def sync_single_stock_financial_data(
    code: str, provider: AKShareProvider, db
) -> bool:
    """
    同步单只股票的财务数据
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"   获取到 {len(df)} 期财务数据，最新期: {latest.get('报告期', 'N/A')}"
            )
        ...
        financial_data = {
            ...
            "total_hldr_eqy_exc_min_int": _safe_float(
                latest.get("股东权益合计")
            ),  # 净资产
            # 每股指标
            ...
            # 运营能力指标
            "total_asset_turnover": _safe_float(
                latest.get("总资产周转率")
            ),  # 总资产周转率
        }
        ...
        if quote and financial_data.get("total_share"):
            ...
            if price:
                ...
                # 计算 PE（优先使用 TTM 净利润）
                net_profit_for_pe = financial_data.get(
                    "net_profit_ttm"
                ...
                # 计算 PS（优先使用 TTM 营业收入）
                revenue_for_ps = financial_data.get(
                    "revenue_ttm"
...
def _calculate_ttm_metric(df, metric_name: str) -> Optional[float]:
    ...
    try:
        ...
        if not last_annual_row.empty:
            logger.warning(
                f"   ⚠️ {metric_name}TTM计算失败: 缺少去年同期数据（需要: {last_same_period}）"
            )
        else:
            logger.warning(
                f"   ⚠️ {metric_name}TTM计算失败: 缺少基准年报（需要: {last_annual_period}）"
            )
...
async def main(
    code: Optional[str] = None, sync_all: bool = False, batch: Optional[int] = None
):
    """主函数"""
    logger.info("=" * 80)
