...
使用方法：
    python scripts/ps/calculation/verification/script.py 600036
    python scripts/ps/calculation/verification/script.py 000001
    python scripts/ps/calculation/verification/script.py 600036 000001 000002
"""
...
class PSCalculationVerifier:
    ...
    async def verify_stock(self, code: str):
        ...
        print(
            f"   营业收入（单期）: {revenue_yuan / 100000000:.2f} 亿元"
            if revenue_yuan
            else "   营业收入（单期）: N/A"
        )
        print(
        ...
        print(
            f"   净资产: {total_equity_yuan / 100000000:.2f} 亿元"
            if total_equity_yuan
            else "   净资产: N/A"
        )
        print(
            f"   总股本: {total_share_yuan / 100000000:.2f} 亿股"
            if total_share_yuan
            else "   总股本: N/A"
        )
        ...
        # 计算市值（亿元）
        market_cap_yi = (
            price * total_share_yuan / 100000000
        )  # 股价（元）× 总股本（股）/ 1亿
...
async def main():
    ...
    if len(sys.argv) < 2:
        print(
            "用法: python scripts/ps/calculation/verification/script.py <股票代码1> [股票代码2] ..."
        )
        print("\n示例:")
        print("  python scripts/ps/calculation/verification/script.py 600036")
        print(
            "  python scripts/ps/calculation/verification/script.py 000001 000002 600036"
        )
        sys.exit(1)
