...
class PSCalculationVerifier:
    ...
    async def connect(self):
        ...
        db_name = os.getenv("MONGODB_DATABASE_NAME") or os.getenv("MONGODB_DATABASE", "trading_agents")
...
async def main():
    """主函数"""
    if len(sys.argv) < 2:
        print("用法: python scripts/ps/calculation/verification/test.py <股票代码1> [股票代码2] ...")
        print("\n示例:")
        print("  python scripts/ps/calculation/verification/test.py 600036")
        print("  python scripts/ps/calculation/verification/test.py 000001 000002 600036")
        sys.exit(1)
