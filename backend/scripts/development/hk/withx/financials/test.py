        ...
        for symbol in test_symbols:
            ...
            try:
                ...
                if indicators:
                    ...
                    print(
                        f"      - 营业收入: {indicators.get('operate_income') / 1e8:.2f} 亿港元"
                    )
                    print(
                        f"      - 营收同比: {indicators.get('operate_income_yoy'):.2f}%"
                    )
                    print(
                        f"      - 资产负债率: {indicators.get('debt_asset_ratio'):.2f}%"
                    )
                else:
                    print("   ⚠️ 未获取到财务指标")
...
def test_pe_pb_calculation():
    ...
    try:
        ...
        if eps_ttm and eps_ttm > 0:
            ...
            print(
                f"   计算的 PE: {pe_calculated:.2f} (= {current_price:.2f} / {eps_ttm:.2f})"
            )
        ...
        if bps and bps > 0:
            ...
            print(
                f"   计算的 PB: {pb_calculated:.2f} (= {current_price:.2f} / {bps:.2f})"
            )
