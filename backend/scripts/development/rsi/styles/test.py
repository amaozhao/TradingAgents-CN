...
from trader.tools.analysis.indicators import add_all_indicators, rsi
...
def test_rsi_methods():
    ...
    print(
        f"✅ 添加的指标列: {[col for col in df_int.columns if col not in ['date', 'close']]}"
    )
    ...
    print(
        f"✅ 添加的指标列: {[col for col in df_china.columns if col not in ['date', 'close']]}"
    )
    ...
    print("=" * 80)
    print(
        f"EMA vs SMA 差异: {abs(df['rsi_ema'].iloc[-1] - df['rsi_sma'].iloc[-1]):.2f}"
    )
    print(
        f"China RSI6 vs RSI12 差异: {abs(df['rsi6_china'].iloc[-1] - df['rsi12_china'].iloc[-1]):.2f}"
    )
    print(
...
def test_a_stock_compatibility():
    ...
    print(
        f"   差异:              {abs(df1['rsi6'].iloc[-1] - df2['rsi6_manual'].iloc[-1]):.6f}"
    )
    ...
    print(
        f"   差异:              {abs(df1['rsi12'].iloc[-1] - df2['rsi12_manual'].iloc[-1]):.6f}"
    )
    ...
    print(
        f"   差异:              {abs(df1['rsi24'].iloc[-1] - df2['rsi24_manual'].iloc[-1]):.6f}"
    )
