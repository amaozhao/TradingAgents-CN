...
async def run_scoped_initialization(service, sync_items):
    """运行小样本真实同步，避免全量测试触发全市场初始化。"""
    return await asyncio.wait_for(
        service.run_full_initialization(
...
async def test_selective_sync():
    ...
    try:
        ...
        print("\n3️⃣ 测试1: 仅同步历史数据（日线）")
        print(
            "   命令: python cli/akshare_init.py --full --sync-items historical --historical-days 7"
        )
        service = await get_akshare_init_service()

        result1 = await run_scoped_initialization(service, ["historical"])
        ...
        print("\n4️⃣ 测试2: 仅同步周线数据")
        print(
            "   命令: python cli/akshare_init.py --full --sync-items weekly --historical-days 7"
        )

        result2 = await run_scoped_initialization(service, ["weekly"])
        ...
        print("\n5️⃣ 测试3: 同步财务数据和行情数据")
        print(
            "   命令: python cli/tushare_init.py --full --sync-items financial,quotes"
        )

        result3 = await run_scoped_initialization(service, ["financial", "quotes"])

        print(f"   ✅ 同步完成: {result3['success']}")
        print(f"   ⏱️  耗时: {result3['duration']:.2f}秒")
        assert result3["success"], result3.get("errors")
        ...
        print("\n📝 CLI使用示例:")
        print("   # 仅同步历史数据")
        print("   python cli/akshare_init.py --full --sync-items historical")
        print()
        print("   # 仅同步财务数据")
        print("   python cli/akshare_init.py --full --sync-items financial")
        print()
        print("   # 同步周线和月线数据")
        print("   python cli/akshare_init.py --full --sync-items weekly,monthly")
        print()
        print("   # 同步多个数据类型")
        print(
            "   python cli/akshare_init.py --full --sync-items historical,financial,quotes"
        )
        print()
    finally:
