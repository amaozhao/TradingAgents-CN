...
async def test_selective_sync():
    ...
    print("\n3️⃣ 测试1: 仅同步历史数据（日线）")
    print(
        "   命令: python cli/tushare_init.py --full --sync-items historical --historical-days 30"
    )
    service = await get_tushare_init_service()

    result1 = await service.run_full_initialization(
        historical_days=30, skip_if_exists=False, sync_items=["historical"]
    )
    ...
    print("\n4️⃣ 测试2: 仅同步周线数据")
    print(
        "   命令: python cli/tushare_init.py --full --sync-items weekly --historical-days 30"
    )

    result2 = await service.run_full_initialization(
        historical_days=30, skip_if_exists=False, sync_items=["weekly"]
    )
