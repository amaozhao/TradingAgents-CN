"""
测试财务数据降级逻辑
验证当 PostgreSQL 没有数据时，是否能正确降级到 AKShare
"""
...
def test_financial_fallback():
    ...
    try:
        ...
        # 先检查 PostgreSQL 中是否有数据
        print(f"\n🔍 步骤2: 检查 PostgreSQL 中是否有 {test_symbol} 的财务数据...")
        ...
        if "估算值" in report or "估算数据" in report:
            print("⚠️ 报告使用了估算值")
            print("   说明所有数据源（PostgreSQL、AKShare、Tushare）都未能获取到数据")
        ...
        # 总结
        print("\n📊 降级逻辑测试总结:")
        print("1. ✅ 系统能够检测 PostgreSQL 中是否有数据")
        print("2. ✅ 当 PostgreSQL 没有数据时，自动降级到 AKShare")
