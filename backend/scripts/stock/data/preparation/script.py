...
def test_stock_data_preparation():
    ...
    try:
        ...
        get_stock_preparation_message = getattr(
            importlib.import_module("trader.utils.validation"),
            "get_stock_preparation_message",
        )
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            message = get_stock_preparation_message(
                test_case["code"], test_case["market"], 30
            )
...
def test_format_validation():
    ...
    try:
        ...
        for i, test in enumerate(format_tests, 1):
            ...
            # 格式错误应该在数据获取前就被拦截
            format_passed = not (
                result.error_message and "格式错误" in result.error_message
            )
...
def test_performance():
    ...
    try:
        ...
        for test in performance_tests:
            ...
            start_time = time.time()
            prepare_stock_data(
                test["code"], test["market"], period_days=7
            )  # 较短时间测试
            end_time = time.time()
