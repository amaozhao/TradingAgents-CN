...
def _build_checks(*, auth_available: bool, allow_missing_auth: bool) -> list[ApiCheck]:
    stock_code = os.getenv("TRADINGAGENTS_SMOKE_STOCK_CODE", "000001")
    symbol = os.getenv("TRADINGAGENTS_SMOKE_SYMBOL", stock_code)
    search_query = os.getenv("TRADINGAGENTS_SMOKE_SEARCH_QUERY", stock_code)
    task_id = os.getenv("TRADINGAGENTS_SMOKE_TASK_ID")
    expected_migration_settings = _expected_migration_settings()

    auth_skip = None
    if not auth_available:
        auth_skip = "missing authentication; provide TRADINGAGENTS_API_TOKEN or username/password"
    ...
    if task_id:
        ...
        checks.append(
            ApiCheck(
                ...
                optional=True,
                skip_reason="TRADINGAGENTS_SMOKE_TASK_ID is not set",
            )
        )
    return checks
...
def _expected_migration_settings() -> dict[str, bool]:
    ...
    for env_name, setting_name in (
        ("TRADINGAGENTS_EXPECT_POSTGRES_READ_ENABLED", "POSTGRES_READ_ENABLED"),
        ("TRADINGAGENTS_EXPECT_POSTGRES_DUAL_WRITE_ENABLED", "POSTGRES_DUAL_WRITE_ENABLED"),
    ):
...
def main() -> None:
    ...
    parser.add_argument("--base-url", default=os.getenv("TRADINGAGENTS_API_BASE_URL", "http://127.0.0.1:8000"))
    parser.add_argument("--token", default=os.getenv("TRADINGAGENTS_API_TOKEN"))
    parser.add_argument("--username", default=os.getenv("TRADINGAGENTS_API_USERNAME"))
    parser.add_argument("--password", default=os.getenv("TRADINGAGENTS_API_PASSWORD"))
    parser.add_argument("--timeout", type=int, default=int(os.getenv("TRADINGAGENTS_API_TIMEOUT", DEFAULT_TIMEOUT_SECONDS)))
