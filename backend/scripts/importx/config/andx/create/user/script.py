...
功能：
1. 从导出的 JSON 文件导入配置数据到 PostgreSQL document store
2. 创建默认管理员用户（admin/admin123）
3. 支持选择性导入集合
4. 支持覆盖或跳过已存在的数据

使用方法：
    python scripts/importx/config/andx/create/user/script.py <export_file.json>
    python scripts/importx/config/andx/create/user/script.py <export_file.json> --overwrite
    python scripts/importx/config/andx/create/user/script.py <export_file.json> --collections system_configs users
"""
...
def load_env_config(script_dir: Path) -> dict:
    ...
    config = {
        ...
        "postgres_database": "trading_agents_cn",
                ...
                for line in f:
                    ...
                    if "=" in line:
                        ...
                        if key == "POSTGRES_PORT":
                            ...
                            config["postgres_database"] = value
...
def _build_unique_query(
    doc: Dict[str, Any], collection_name: str
...
def convert_to_document_ids(data: Any) -> Any:
    """将 JSON 数据转换为 JSON 兼容格式"""
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            ...
            result[key] = convert_to_document_ids(value)
        return result

    elif isinstance(data, list):
        return [convert_to_document_ids(item) for item in data]
...
def connect_postgres(use_docker: bool = True, config: dict = None) -> Any:
    """连接到 PostgreSQL document store

    Args:
        use_docker: 兼容旧参数，当前不再影响连接方式
        config: 兼容旧参数，当前不再影响连接方式
    """
    if config is None:
        config = {
            "postgres_port": 5432,
    ...
    print(f"\n🔌 连接到 PostgreSQL document store ({env_name})...")
    print("   数据库: trading_agents_cn")
    ...
    try:
        ...
        print(f"❌ 错误: PostgreSQL document store 连接失败: {e}")
        sys.exit(1)
...
def import_collection(
    ...
    # 转换文档格式
    converted_docs = [convert_to_document_ids(doc) for doc in documents]
...
示例:
  # 在 Docker 容器内运行（默认）
  python scripts/importx/config/andx/create/user/script.py

  # 在宿主机运行（连接到 localhost:5432）
  python scripts/importx/config/andx/create/user/script.py --host

  # 从指定文件导入（默认覆盖模式）
  python scripts/importx/config/andx/create/user/script.py export.json

  # 增量模式：跳过已存在的数据
  python scripts/importx/config/andx/create/user/script.py --incremental

  # 只导入指定的集合
  python scripts/importx/config/andx/create/user/script.py --collections system_configs users

  # 只创建默认用户，不导入数据
  python scripts/importx/config/andx/create/user/script.py --create-user-only
        """,
    )
    ...
    parser.add_argument(
        "--host",
        action="store_true",
        help="在宿主机运行（连接 localhost:5432），默认在 Docker 容器内运行（连接 postgres:5432）",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    parser.add_argument(
        "--incremental", action="store_true", help="增量模式：跳过已存在的数据"
    )
    parser.add_argument(
        "--collections", nargs="+", help="指定要导入的集合（默认：所有配置集合）"
    )
    parser.add_argument(
        "--create-user-only", action="store_true", help="只创建默认用户，不导入数据"
    )
    parser.add_argument("--skip-user", action="store_true", help="跳过创建默认用户")
    parser.add_argument(
        "--postgres-port", type=int, help="PostgreSQL 端口（覆盖 .env 配置）"
    )
    parser.add_argument(
        "--postgres-host", type=str, help="PostgreSQL 主机（覆盖 .env 配置）"
    )
    ...
    if not args.create_user_only and not args.export_file:
        install_dir = BACKEND_ROOT / "install"
    ...
    if args.postgres_host:
        ...
        env_config["postgres_connection_string"] = None

    # 连接数据库
    use_docker = not args.host  # 默认在 Docker 内运行，除非指定 --host
    client = connect_postgres(use_docker=use_docker, config=env_config)
    db_name = env_config["postgres_database"]
    db = client[db_name]
    ...
    if not args.create_user_only:
        ...
        for collection_name in collections_to_import:
            ...
            try:
                stats = import_collection(
                    db, collection_name, documents, args.overwrite
                )
                ...
                if args.overwrite:
                    print(
                        f"      ✅ 删除 {stats['deleted']} 个，插入 {stats['inserted']} 个"
                    )
                else:
                    print(
                        f"      ✅ 插入 {stats['inserted']} 个，跳过 {stats['skipped']} 个"
                    )
    ...
    print("\n📝 后续步骤:")
    print("   1. 重启后端服务: docker restart trading-agents-backend")
