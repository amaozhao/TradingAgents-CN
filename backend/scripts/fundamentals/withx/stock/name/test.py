...
def test_fundamentals_with_fallback():
    ...
    try:
        ...
        fundamentals_report = analyzer._generate_fundamentals_report(
            fake_code, stock_data
        )

        # 3. 检查是否使用了降级机制
        if (
            "数据来源: akshare" in fundamentals_report
            or "数据来源: baostock" in fundamentals_report
        ):
...
def test_complete_fundamentals_flow():
    ...
    try:
        # 模拟完整的基本面分析调用
        AgentUtils = getattr(
            importlib.import_module("trader.agents.utils.utils"), "AgentUtils"
        )
