"""
测试脚本：验证从 PostgreSQL 读取模型配置

这个脚本会：
1. 测试从 PostgreSQL 读取模型配置
2. 验证字符串到枚举的转换
3. 测试模型验证逻辑
"""
...
async def test_postgres_config():
    """测试 PostgreSQL 配置"""

    print("=" * 80)
    print("测试：从 PostgreSQL 读取模型配置")
    print("=" * 80)
...
def test_model_config_service():
    ...
    ModelCapabilityService = getattr(
        importlib.import_module("app.services.capability"), "ModelCapabilityService"
    )
    ModelFeature = getattr(
        importlib.import_module("app.constants.capabilities"), "ModelFeature"
    )
    ModelRole = getattr(
        importlib.import_module("app.constants.capabilities"), "ModelRole"
    )
...
if __name__ == "__main__":
    # 测试 PostgreSQL 配置
    asyncio.run(test_postgres_config())
