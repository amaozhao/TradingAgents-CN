...
if ttm_net_profit and abs(ttm_net_profit - expected_ttm_profit) < 0.01:
    ...
    print(
        f"❌ TTM 净利润计算错误！期望: {expected_ttm_profit:.2f}，实际: {ttm_net_profit:.2f}"
    )
...
print(
    f"年报 TTM 营业收入: {ttm_revenue_annual:.2f} 万元"
    if ttm_revenue_annual
    else "计算失败"
)
print(
    f"年报 TTM 净利润: {ttm_profit_annual:.2f} 万元"
    if ttm_profit_annual
    else "计算失败"
)
