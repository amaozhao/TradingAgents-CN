...
DEFAULT_OUTPUT_ROOT = Path("/tmp/tradingagents_postgres_cutover_evidence")
...
def build_gate_steps(
    *,
    ...
    query_plan_command = [python, "backend/scripts/postgres_query_plan_check.py"]
    if compile_only_query_plan:
        query_plan_command.append("--compile-only")
    runtime_log_command = [python, "backend/scripts/postgres_runtime_log_check.py"]
    ...
    steps = [
        GateStep(
            name="inventory",
            command=[python, "backend/scripts/postgres_migration_inventory.py"],
        ...
        GateStep(
            ...
            command=[python, "backend/scripts/postgres_cutover_smoke.py", "--pretty"],
    ...
    if include_api_smoke:
        steps.append(
            GateStep(
                name="api_smoke",
                command=[python, "backend/scripts/postgres_api_smoke.py", "--pretty"],
...
def _build_target_manifest(
    *,
    ...
    return {
        "schema_version": "1",
        "created_at": datetime.now(UTC).isoformat(),
        "target_env": target_env or os.getenv("TRADINGAGENTS_TARGET_ENV"),
        "target_phase": target_phase or os.getenv("TRADINGAGENTS_CUTOVER_PHASE"),
        "output_dir": str(output_dir),
        "include_api_smoke": include_api_smoke,
        ...
        "expected_postgres_read_enabled": os.getenv("TRADINGAGENTS_EXPECT_POSTGRES_READ_ENABLED"),
        "expected_postgres_dual_write_enabled": os.getenv("TRADINGAGENTS_EXPECT_POSTGRES_DUAL_WRITE_ENABLED"),
...
def _safe_environment_snapshot() -> dict[str, str | bool | None]:
    return {
        ...
        "TRADINGAGENTS_API_BASE_URL_SET": bool(os.getenv("TRADINGAGENTS_API_BASE_URL")),
        "TRADINGAGENTS_API_TOKEN_SET": bool(os.getenv("TRADINGAGENTS_API_TOKEN")),
        "TRADINGAGENTS_API_USERNAME_SET": bool(os.getenv("TRADINGAGENTS_API_USERNAME")),
        "TRADINGAGENTS_API_PASSWORD_SET": bool(os.getenv("TRADINGAGENTS_API_PASSWORD")),
    }
...
def _validate_explicit_target_environment(
    *,
    ...
    missing: list[str] = []
    if not (target_env or os.getenv("TRADINGAGENTS_TARGET_ENV")):
        missing.append("TRADINGAGENTS_TARGET_ENV or --target-env")
    effective_phase = target_phase or os.getenv("TRADINGAGENTS_CUTOVER_PHASE")
    if not effective_phase:
        missing.append("TRADINGAGENTS_CUTOVER_PHASE or --target-phase")
    elif effective_phase not in {"pre-read", "post-read", "rollback"}:
        missing.append("TRADINGAGENTS_CUTOVER_PHASE valid value or --target-phase")
    ...
    if include_api_smoke:
        _require_env(missing, "TRADINGAGENTS_API_BASE_URL")
        has_token = bool(os.getenv("TRADINGAGENTS_API_TOKEN"))
        has_login = bool(os.getenv("TRADINGAGENTS_API_USERNAME")) and bool(os.getenv("TRADINGAGENTS_API_PASSWORD"))
        if not has_token and not has_login:
            missing.append("TRADINGAGENTS_API_TOKEN or TRADINGAGENTS_API_USERNAME+TRADINGAGENTS_API_PASSWORD")
        _require_env(missing, "TRADINGAGENTS_EXPECT_POSTGRES_READ_ENABLED")
        _require_env(missing, "TRADINGAGENTS_EXPECT_POSTGRES_DUAL_WRITE_ENABLED")
