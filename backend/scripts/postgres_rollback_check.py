...
def write_rollback_target_manifest(
    *,
    ...
    environment = env if env is not None else os.environ
    effective_target_env = target_env or environment.get("TRADINGAGENTS_TARGET_ENV")
    if not effective_target_env:
        raise ValueError("missing target environment: provide --target-env or TRADINGAGENTS_TARGET_ENV")
    manifest = {
        "schema_version": "1",
        ...
        "expected_postgres_read_enabled": environment.get("TRADINGAGENTS_EXPECT_POSTGRES_READ_ENABLED"),
        "expected_postgres_dual_write_enabled": environment.get("TRADINGAGENTS_EXPECT_POSTGRES_DUAL_WRITE_ENABLED"),
