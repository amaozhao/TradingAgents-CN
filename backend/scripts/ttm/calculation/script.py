...
from scripts.sync.financial.data.script import _calculate_ttm_revenue
...
def test_ttm_calculation():
    ...
    df1 = pd.DataFrame(
        {"报告期": ["20221231", "20231231"], "营业收入": [1000.0, 1200.0]}
    )
    ...
    print("\n【测试4】一季报数据（简单年化）")
    df4 = pd.DataFrame(
        {"报告期": ["20231231", "20240331"], "营业收入": [1000.0, 300.0]}
    )
    ...
    print("\n【测试5】三季报数据（简单年化）")
    df5 = pd.DataFrame(
        {"报告期": ["20231231", "20240930"], "营业收入": [1000.0, 900.0]}
    )
