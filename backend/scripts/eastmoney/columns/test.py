...
def main() -> None:
    ...
    print("\n📊 市场分布:")
    print(
        f"  60开头(沪市主板): {len(df[df['代码'].str.startswith('60', na=False)])} 只"
    )
    print(
        f"  00开头(深市主板): {len(df[df['代码'].str.startswith('00', na=False)])} 只"
    )
    print(f"  30开头(创业板): {len(df[df['代码'].str.startswith('30', na=False)])} 只")
    print(f"  68开头(科创板): {len(df[df['代码'].str.startswith('68', na=False)])} 只")
    print(
        f"  43/83/87开头(北交所): {len(df[df['代码'].str.match(r'^(43|83|87)', na=False)])} 只"
    )
