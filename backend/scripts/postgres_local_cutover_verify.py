...
def _safe_env_snapshot(env: dict[str, str]) -> dict[str, str | bool]:
    return {
        ...
        "TRADINGAGENTS_API_BASE_URL_SET": bool(env.get("TRADINGAGENTS_API_BASE_URL")),
        "TRADINGAGENTS_API_TOKEN_SET": bool(env.get("TRADINGAGENTS_API_TOKEN")),
    }
...
async def run_local_cutover_verification(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.api_base_url:
        env["TRADINGAGENTS_API_BASE_URL"] = args.api_base_url
    if args.api_token:
        env["TRADINGAGENTS_API_TOKEN"] = args.api_token
