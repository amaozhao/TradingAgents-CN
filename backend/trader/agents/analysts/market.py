...
def _get_company_name(ticker: str, market_info: dict) -> str:
    ...
    try:
        if market_info["is_china"]:
            ...
            logger.debug(
                f"📊 [市场分析师] 获取股票信息返回: {stock_info[:200] if stock_info else 'None'}..."
            )
            ...
            if stock_info and "股票名称:" in stock_info:
                ...
                logger.info(
                    f"✅ [市场分析师] 成功获取中国股票名称: {ticker} -> {company_name}"
                )
                return company_name
            else:
                # 降级方案：尝试直接从数据源管理器获取
                logger.warning(
                    f"⚠️ [市场分析师] 无法从统一接口解析股票名称: {ticker}，尝试降级方案"
                )
                try:
                    ...
                    if info_dict and info_dict.get("name"):
                        ...
                        logger.info(
                            f"✅ [市场分析师] 降级方案成功获取股票名称: {ticker} -> {company_name}"
                        )
                        return company_name
    ...
    def market_analyst_node(state):
        ...
        if GoogleToolCallHandler.is_google_model(llm):
            ...
            if len(result.tool_calls) == 0:
                ...
                logger.info(
                    f"📊 [市场分析师] ✅ 直接回复（无工具调用），长度: {len(report)}"
                )
