...
专注于可操作的见解和持续改进。建立在过去经验教训的基础上，批判性地评估所有观点，确保每个决策都能带来更好的结果。请用中文撰写所有分析内容和建议。"""
        ...
        while retry_count < max_retries:
            try:
                logger.info(
                    f"🔄 [Risk Manager] 调用LLM生成交易决策 (尝试 {retry_count + 1}/{max_retries})"
                )
...
"""
                ...
                if response and hasattr(response, "content") and response.content:
                    ...
                    if len(response_content) > 10:  # 确保响应有实质内容
                        ...
                        logger.warning(
                            f"⚠️ [Risk Manager] LLM响应内容过短: {len(response_content)} 字符"
                        )
                        response_content = ""
                else:
                ...
                elapsed_time = time.time() - start_time
                logger.error(
                    f"❌ [Risk Manager] LLM调用失败 (尝试 {retry_count + 1}): {str(e)}"
                )
...
注意：此为系统默认建议，建议结合人工分析做出最终决策。"""
        ...
        new_risk_debate_state = {
            ...
            "aggressive_history": risk_debate_state.get(
                "aggressive_history", risk_debate_state["risky_history"]
            ),
            "safe_history": risk_debate_state["safe_history"],
            "conservative_history": risk_debate_state.get(
                "conservative_history", risk_debate_state["safe_history"]
            ),
            ...
            "current_aggressive_response": risk_debate_state.get(
                ...
                risk_debate_state["current_risky_response"],
            ),
            "current_safe_response": risk_debate_state["current_safe_response"],
            "current_conservative_response": risk_debate_state.get(
                "current_conservative_response",
                risk_debate_state["current_safe_response"],
            ),
        ...
        logger.info(
            f"📋 [Risk Manager] 最终决策生成完成，内容长度: {len(response_content)} 字符"
        )
