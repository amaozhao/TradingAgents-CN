...
class ChatDashScopeOpenAI(ChatOpenAI):
    ...
    def __init__(self, **kwargs):
        ...
        logger.info(
            f"🔍 [DashScope初始化] kwargs 中是否包含 api_key: {'api_key' in kwargs}"
        )
        ...
        if not api_key_from_kwargs:
            ...
            if env_api_key and is_valid_api_key(env_api_key):
                logger.info(
                    f"✅ [DashScope初始化] 环境变量中的 API Key 有效，长度: {len(env_api_key)}, 前10位: {env_api_key[:10]}..."
                )
                api_key_from_kwargs = env_api_key
            elif env_api_key:
                logger.warning(
                    "⚠️ [DashScope初始化] 环境变量中的 API Key 无效（可能是占位符），将被忽略"
                )
            ...
            logger.info(
                "✅ [DashScope初始化] 使用 kwargs 中传入的 API Key（来自数据库配置）"
            )
        ...
        logger.info(
            f"🔍 [DashScope初始化] 最终使用的 API Key: {'有值' if final_api_key else '空'}"
        )
            ...
            if hasattr(result, "llm_output") and result.llm_output:
                ...
                if input_tokens > 0 or output_tokens > 0:
                    ...
                    session_id = kwargs.get(
                        "session_id", f"dashscope_openai_{hash(str(args)) % 10000}"
                    )
