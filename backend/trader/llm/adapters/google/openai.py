...
class ChatGoogleOpenAI(ChatGoogleGenerativeAI):
    ...
    def __init__(self, base_url: Optional[str] = None, **kwargs):
        ...
        logger.info(
            f"🔍 [Google初始化] kwargs 中是否包含 google_api_key: {'google_api_key' in kwargs}"
        )
        ...
        if not google_api_key:
            ...
            if env_api_key and is_valid_api_key(env_api_key):
                logger.info(
                    f"✅ [Google初始化] 环境变量中的 API Key 有效，长度: {len(env_api_key)}, 前10位: {env_api_key[:10]}..."
                )
                google_api_key = env_api_key
            elif env_api_key:
                logger.warning(
                    "⚠️ [Google初始化] 环境变量中的 API Key 无效（可能是占位符），将被忽略"
                )
                google_api_key = None
            else:
            ...
            logger.info(
                "✅ [Google初始化] 使用 kwargs 中传入的 API Key（来自数据库配置）"
            )

        logger.info(
            f"🔍 [Google初始化] 最终使用的 API Key: {'有值' if google_api_key else '空'}"
        )
        ...
        if base_url:
            ...
            if is_google_official:
                ...
                if base_url.endswith("/v1beta"):
                    ...
                    logger.info(
                        f"🔍 [Google官方] 使用完整 base_url 作为域名: {api_endpoint}"
                    )
                ...
                api_endpoint = base_url
                logger.info(
                    f"🔄 [中转地址] 检测到非官方域名，使用完整 URL: {api_endpoint}"
                )
            ...
            logger.info(
                f"✅ [Google初始化] 设置 client_options.api_endpoint: {api_endpoint}"
            )
        else:
            ...
            if hasattr(result, "llm_output") and result.llm_output:
                ...
                if input_tokens > 0 or output_tokens > 0:
                    ...
                    session_id = kwargs.get(
                        "session_id", f"google_openai_{hash(str(kwargs)) % 10000}"
                    )
                    ...
                    logger.debug(
                        f"📊 [Google适配器] Token使用量: 输入={input_tokens}, 输出={output_tokens}"
                    )
