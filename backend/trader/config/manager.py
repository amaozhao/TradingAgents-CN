...
# 导入数据模型（避免循环导入）
from .usage import UsageRecord, ModelConfig, PricingConfig
...
class ConfigManager:
    ...
    def _init_mongodb_storage(self):
        ...
        try:
            ...
            database_name = os.getenv("MONGODB_DATABASE_NAME", "trading_agents")
...
def _get_project_config_dir():
    """获取项目根目录的配置目录"""
    # 从当前文件位置推断项目根目录
    current_file = Path(__file__)  # trader/config/config_manager.py
    project_root = current_file.resolve().parents[3]  # backend/trader/config -> 仓库根目录
