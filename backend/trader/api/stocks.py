...
try:
    from trader.flows.service import get_stock_data_service
...
def get_all_stocks() -> List[Dict[str, Any]]:
    ...
    if result is None or (isinstance(result, dict) and "error" in result):
        return [
            {"error": "无法获取股票列表", "suggestion": "请检查网络连接和数据库配置"}
        ]
...
def get_market_summary() -> Dict[str, Any]:
    ...
    return {
        ...
        "data_source": all_stocks[0].get("source", "unknown")
        if all_stocks
        else "unknown",
...
def check_service_status() -> Dict[str, Any]:
    ...
    Example:
        ...
        logger.info(f"PostgreSQL状态: {status["postgres_status']}")
    """
    if not SERVICE_AVAILABLE:
        return {
            "service_available": False,
    ...
    if service.db_manager:
        try:
            # 尝试检查数据库管理器的连接状态
            if (
                hasattr(service.db_manager, "is_postgres_available")
                and service.db_manager.is_postgres_available()
            ):
                postgres_status = "connected"
            elif (
                hasattr(service.db_manager, "postgres_client")
                and service.db_manager.postgres_client
            ):
                # 尝试执行一个简单的查询来测试连接
                service.db_manager.postgres_client.admin.command("ping")
                postgres_status = "connected"
            else:
