...
def __getattr__(name):
    ...
    if name == "config_manager":
        config_manager = getattr(importlib.import_module('trader.config'), 'config_manager')

        return config_manager
    if name == "logging_manager":
        logging_manager = getattr(importlib.import_module('trader.utils.logging'), 'manager')
