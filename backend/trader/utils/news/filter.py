...
class NewsRelevanceFilter:
    ...
    def calculate_relevance_score(self, title: str, content: str) -> float:
        ...
        if include_matches:
            logger.debug(
                f"[过滤器] 相关关键词匹配: {include_matches[:3]}..."
            )  # 只显示前3个
    ...
    def filter_news(self, news_df: pd.DataFrame, min_score: float = 30) -> pd.DataFrame:
        ...
        logger.info(
            f"[过滤器] 开始过滤新闻，原始数量: {len(news_df)}条，最低评分阈值: {min_score}"
        )
    ...
    def get_filter_statistics(
        self, original_df: pd.DataFrame, filtered_df: pd.DataFrame
    ) -> Dict:
        """
        获取过滤统计信息
        ...
        stats = {
            ...
            "filter_rate": (len(original_df) - len(filtered_df))
            / len(original_df)
            * 100
...
def get_company_name(ticker: str) -> str:
    ...
    if company_name:
        ...
        logger.warning(
            f"[公司映射] 未找到 {ticker} 的公司名称映射，使用默认: {default_name}"
        )
        return default_name
