            ...
            if env_token:
                try:
                    ...
                    if test_data is not None and not test_data.empty:
                        ...
                        self.logger.info(
                            "✅ [步骤4.2] Tushare连接成功 (Token来源: .env 环境变量)"
                        )
                        return True
                    else:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if db_token:
                try:
                    self.logger.info(
                        f"🔄 尝试使用数据库中的 Tushare Token (超时: {test_timeout}秒)..."
                    )
                    ts.set_token(db_token)
                    self.api = ts.pro_api()
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"⚠️ 数据库 Token 测试超时 ({test_timeout}秒)，尝试降级到 .env 配置..."
                        )
                        test_data = None
