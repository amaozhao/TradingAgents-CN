...
from trader.config.runtime import get_float, get_int, get_timezone_name
...
class HKStockProvider:
    ...
    def get_stock_data(
        ...
        try:
            ...
            # 设置默认日期
            if not end_date:
                end_date = datetime.now(ZoneInfo(get_timezone_name())).strftime(
                    "%Y-%m-%d"
                )
            if not start_date:
                start_date = (
                    datetime.now(ZoneInfo(get_timezone_name())) - timedelta(days=365)
                ).strftime("%Y-%m-%d")
        ...
        try:
            ...
            if not data.empty:
                ...
                return {
                    ...
                    "timestamp": latest_index.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def format_stock_data(
        ...
        try:
            ...
            logger.info(
                f"📊 [港股技术指标] 开始计算技术指标，原始数据: {original_data_count}条"
            )
            ...
            logger.info(
                f"🔍 [港股技术指标详情] ===== 最近{display_rows}个交易日数据 ====="
            )
            ...
            if not pd.isna(ma5):
                ...
                result += (
                    f"   MA5: HK${ma5:.2f} (价格在MA5{ma5_pos} {abs(ma5_diff):.2f}%)\n"
                )
            ...
            if (
                not pd.isna(boll_upper)
                and not pd.isna(boll_mid)
                and not pd.isna(boll_lower)
            ):
            ...
            logger.info(
                f"✅ [港股技术指标] 技术指标计算完成，展示最后{display_rows}天数据"
            )
