...
"""
PostgreSQL 缓存适配器
根据 TA_USE_APP_CACHE 配置，优先使用 PostgreSQL document store 中的同步数据
"""
...
class PostgresCacheAdapter:
    """PostgreSQL 缓存适配器（从 app 的 PostgreSQL document store 读取同步数据）"""
    ...
    def __init__(self):
        ...
        if self.use_app_cache:
            ...
            logger.info("🔄 PostgreSQL缓存适配器已启用 - 优先使用PostgreSQL数据")
        else:
            logger.info("📁 PostgreSQL缓存适配器使用传统缓存模式")

    def _init_postgres_connection(self):
        """初始化PostgreSQL document store 连接"""
        try:
            ...
            if self.postgres_client:
                self.db = self.postgres_client.get_database("trading_agents")
                logger.debug("✅ PostgreSQL document store 连接初始化成功")
            else:
                logger.warning(
                    "⚠️ PostgreSQL document store 客户端不可用，回退到传统模式"
                )
                self.use_app_cache = False
        except Exception as e:
            logger.warning(f"⚠️ PostgreSQL document store 连接初始化失败: {e}")
            self.use_app_cache = False
    ...
    def get_stock_basic_info(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for src in source_priority:
                ...
                if doc:
                    logger.debug(
                        f"✅ 从PostgreSQL document store 获取基础信息: {symbol}, 数据源: {src}"
                    )
                    return doc
            ...
            if not doc:
                ...
                if doc:
                    logger.debug(
                        f"✅ 从PostgreSQL document store 获取基础信息（旧数据）: {symbol}"
                    )
                    return doc
                else:
                    logger.debug(
                        f"📊 PostgreSQL document store 中未找到基础信息: {symbol}"
                    )
                    return None
    ...
    def _get_data_source_priority(self, symbol: str) -> list:
        ...
        logger.info(
            "📊 [数据源优先级] 使用默认顺序: ['tushare', 'akshare', 'baostock']"
        )
    ...
    def get_historical_data(
        ...
        try:
            ...
            for data_source in priority_order:
                ...
                # 查询数据
                logger.debug(
                    f"🔍 [ PostgreSQL查询] 尝试数据源: {data_source}, symbol={code6}, period={period}"
                )
                ...
                if data:
                    ...
                    logger.info(
                        f"✅ [数据来源:  PostgreSQL-{data_source}] {symbol}, {len(df)}条记录 (period={period})"
                    )
                    return df
                else:
                    logger.debug(
                        f"⚠️ [ PostgreSQL-{data_source}] 未找到{period}数据: {symbol}"
                    )

            # 所有数据源都没有数据
            logger.warning(
                f"⚠️ [数据来源:  PostgreSQL] 所有数据源({', '.join(priority_order)})都没有{period}数据: {symbol}，降级到其他数据源"
            )
            return None
    ...
    def get_financial_data(
        self, symbol: str, report_period: Optional[str] = None
        ...
        try:
            ...
            for data_source in priority_order:
                ...
                if doc:
                    logger.info(
                        f"✅ [数据来源:  PostgreSQL-{data_source}] {symbol}财务数据"
                    )
                    logger.debug(
            ...
            # 所有数据源都没有数据
            logger.debug(
                f"📊 [数据来源:  PostgreSQL] 所有数据源都没有财务数据: {symbol}"
            )
            return None

        except Exception as e:
            logger.warning(f"⚠️ [数据来源:  PostgreSQL-财务数据] 获取财务数据失败: {e}")
            return None
    ...
    def get_news_data(
        ...
        try:
            ...
            if data:
                logger.debug(
                    f"✅ [数据来源:  PostgreSQL-新闻数据] 从PostgreSQL document store 获取新闻数据: {len(data)}条"
                )
                return data
            else:
                logger.debug(
                    "📊 [数据来源:  PostgreSQL-新闻数据] PostgreSQL document store 中未找到新闻数据"
                )
                return None

        except Exception as e:
            logger.warning(f"⚠️ [数据来源:  PostgreSQL-新闻数据] 获取新闻数据失败: {e}")
            return None
    ...
    def get_social_media_data(
        ...
        try:
            ...
            if data:
                logger.debug(
                    f"✅ 从PostgreSQL document store 获取社媒数据: {len(data)}条"
                )
                return data
            else:
                logger.debug("📊 PostgreSQL document store 中未找到社媒数据")
                return None
    ...
    def get_market_quotes(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if doc:
                logger.debug(f"✅ 从PostgreSQL document store 获取行情数据: {symbol}")
                return doc
            else:
                logger.debug(f"📊 PostgreSQL document store 中未找到行情数据: {symbol}")
                return None
...
# 全局实例
_postgres_cache_adapter = None


def get_postgres_cache_adapter() -> PostgresCacheAdapter:
    """获取 PostgreSQL 缓存适配器实例"""
    global _postgres_cache_adapter
    if _postgres_cache_adapter is None:
        _postgres_cache_adapter = PostgresCacheAdapter()
    return _postgres_cache_adapter


# 向后兼容的别名
def get_enhanced_data_adapter() -> PostgresCacheAdapter:
    """获取增强数据适配器实例（向后兼容，推荐使用 get_postgres_cache_adapter）"""
...
def get_financial_data_with_fallback(
    ...
    if adapter.use_app_cache:
        ...
        if data:
            logger.info(f"💰 使用 PostgreSQL财务数据: {symbol}")
            return data
