...
class ChineseFinanceDataAggregator:
    ...
    def _calculate_overall_sentiment(
        self, news_sentiment: Dict, forum_sentiment: Dict, media_sentiment: Dict
    ) -> Dict:
        """计算综合情绪分析"""
        # 根据各数据源的置信度加权计算
    ...
    def _generate_sentiment_summary(self, overall_sentiment: Dict) -> str:
        ...
        confidence_level = (
            "高" if confidence > 0.7 else "中" if confidence > 0.3 else "低"
        )

        return (
            f"市场情绪: {description} (评分: {score:.2f}, 置信度: {confidence_level})"
        )
