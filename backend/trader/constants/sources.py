...
class DataSourceCode(str, Enum):
    ...
    # ==================== 缓存数据源 ====================
    POSTGRES = "postgres"  # PostgreSQL 数据库缓存（最高优先级）
...
DATA_SOURCE_REGISTRY: Dict[str, DataSourceInfo] = {
    # PostgreSQL 缓存
    DataSourceCode.POSTGRES: DataSourceInfo(
        code=DataSourceCode.POSTGRES,
        name="PostgreSQL",
        display_name="PostgreSQL 缓存",
        provider="Local PostgreSQL",
        description="本地 PostgreSQL 数据库缓存，最高优先级数据源",
...
def list_data_sources_by_market(market: str) -> List[DataSourceInfo]:
    ...
    return [
        info
        for info in DATA_SOURCE_REGISTRY.values()
