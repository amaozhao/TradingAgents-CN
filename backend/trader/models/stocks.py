...
"""
股票数据模型定义
定义标准化的股票数据结构，用于PostgreSQL存储和数据交换
"""
...
class BalanceSheetData(BaseModel):
    ...
    total_hldr_eqy_exc_min_int: Optional[float] = Field(
...
class CashflowStatementData(BaseModel):
    ...
    n_cashflow_inv_act: Optional[float] = Field(
        None, description="投资活动现金流量净额"
    )
    n_cashflow_fin_act: Optional[float] = Field(
        None, description="筹资活动现金流量净额"
    )
    c_cash_equ_end_period: Optional[float] = Field(
        None, description="期末现金及现金等价物余额"
    )
    c_cash_equ_beg_period: Optional[float] = Field(
        None, description="期初现金及现金等价物余额"
    )
...
class StockFinancialData(BaseStockModel):
    ...
    cashflow_statement: Optional[CashflowStatementData] = Field(
        None, description="现金流量表数据"
    )
    financial_indicators: Optional[FinancialIndicators] = Field(
        None, description="财务指标"
    )
...
class DataSourceConfig(BaseStockModel):
    ...
    supported_data_types: List[str] = Field(
        default_factory=list, description="支持的数据类型"
    )
    supported_markets: List[MarketType] = Field(
        default_factory=list, description="支持的市场"
    )
