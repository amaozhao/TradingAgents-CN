...
class UnifiedNewsAnalyzer:
    ...
    def _get_news_from_database(self, stock_code: str, max_news: int = 10) -> str:
        ...
        try:
            ...
            if not client:
                logger.warning("[统一新闻工具] 无法连接到PostgreSQL")
                return ""
            ...
            for query in query_list:
                ...
                if news_items:
                    logger.info(
                        f"[统一新闻工具] 📊 使用查询 {query} 找到 {len(news_items)} 条新闻"
                    )
                    break
            ...
            logger.info(
                f"[统一新闻工具] ✅ 成功从数据库获取并格式化 {len(news_items)} 条新闻"
            )
            return report
                    ...
                    async def get_news_task():
                        try:
                            ...
                            news_data = await provider.get_stock_news(
                                symbol=clean_code, limit=max_news
                            )
    ...
    def _get_a_share_news(
        ...
        try:
            ...
            if db_news:
                logger.info(
                    f"[统一新闻工具] ✅ 数据库新闻获取成功: {len(db_news)} 字符"
                )
                return self._format_news_result(db_news, "数据库缓存", model_info)
            else:
                logger.info(
                    f"[统一新闻工具] ⚠️ 数据库中没有 {stock_code} 的新闻，尝试同步..."
                )

                # 🔥 数据库没有数据时，调用同步服务同步新闻
                try:
                    logger.info(
                        f"[统一新闻工具] 📡 调用同步服务同步 {stock_code} 的新闻..."
                    )
                    ...
                    if synced_news:
                        ...
                        if db_news:
                            logger.info(
                                f"[统一新闻工具] ✅ 同步后数据库新闻获取成功: {len(db_news)} 字符"
                            )
                            return self._format_news_result(
                                db_news, "数据库缓存(新同步)", model_info
                            )
                    else:
        ...
        try:
            if hasattr(self.toolkit, "get_realtime_stock_news"):
                ...
                # 🔍 详细记录东方财富返回的内容
                logger.info(
                    f"[统一新闻工具] 📊 东方财富返回内容长度: {len(result) if result else 0} 字符"
                )
                logger.info(
                    f"[统一新闻工具] 📋 东方财富返回内容预览 (前500字符): {result[:500] if result else 'None'}"
                )
    ...
    def _format_news_result(
        ...
        # 🔍 添加调试日志：打印原始新闻内容
        logger.info(
            f"[统一新闻工具] 📋 原始新闻内容预览 (前500字符): {news_content[:500]}"
        )
        ...
        if is_google_model and len(news_content) > 5000:  # 降低阈值到5000字符
            ...
            for line in lines:
                ...
                if is_important and char_count + len(line) < target_length:
                    ...
                    not is_important and char_count + len(line) < target_length * 0.7
                ):  # 非重要内容更严格限制
                    important_lines.append(line)
                    char_count += len(line)
