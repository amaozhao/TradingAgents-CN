...
from tradingagents.dataflows.akshareutils import get_akshare_provider
...
from tradingagents.dataflows.tushareutils import get_tushare_provider as get_tushare_adapter
...
def _standardize_daily_df(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame()
    result = df.copy()
    result = result.rename(columns={k: v for k, v in _COLUMN_ALIASES.items() if k in result.columns})
...
def _fetch_from_provider(provider_name: str, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
