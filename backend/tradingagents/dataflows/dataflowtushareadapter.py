"""Compatibility adapter for legacy ``tradingagents.dataflows.dataflowtushareadapter`` imports."""
...
class TushareDataAdapter:
    ...
    def _get_realtime_data(self, symbol: str) -> pd.DataFrame:
        if self.enable_cache or use_app_cache_enabled(default=False):
            try:
                from tradingagents.dataflows.appcacheadapter import get_market_quote_dataframe
