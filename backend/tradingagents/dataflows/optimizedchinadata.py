...
# 导入 MongoDB 缓存适配器
from .cache.mongodbcacheadapter import get_mongodb_cache_adapter, get_stock_data_with_fallback, get_financial_data_with_fallback
...
"""
    ...
    def _get_stock_basic_info_only(self, symbol: str) -> str:
        ...
        try:
            ...
            try:
                from tradingagents.config.runtimesettings import use_app_cache_enabled
                if use_app_cache_enabled(False):
                    from .cache.appadapter import get_market_quote_dataframe
    ...
    def _generate_fundamentals_report(self, symbol: str, stock_data: str, analysis_modules: str = "standard") -> str:
        ...
        try:
            if (current_price == "N/A" or change_pct == "N/A" or volume == "N/A"):
                from tradingagents.config.runtimesettings import use_app_cache_enabled  # type: ignore
                if use_app_cache_enabled(False):
                    from .cache.appadapter import get_market_quote_dataframe
                    df_q = get_market_quote_dataframe(symbol)
                    if df_q is not None and not df_q.empty:
...
"""
    ...
    def _get_industry_info(self, symbol: str) -> dict:
        ...
        # 首先尝试从数据库获取真实的行业信息
        try:
            from .cache.appadapter import get_basics_from_cache
    ...
    def _get_real_financial_metrics(self, symbol: str, price_value: float) -> dict:
        ...
        try:
            ...
            from tradingagents.config.databasemanager import get_database_manager
            ...
            if use_app_cache_enabled(False):
                ...
                from tradingagents.dataflows.cache.mongodbcacheadapter import get_mongodb_cache_adapter
                adapter = get_mongodb_cache_adapter()
                financial_data = adapter.get_financial_data(symbol)
    ...
    def _parse_mongodb_financial_data(self, financial_data: dict, price_value: float) -> dict:
        ...
        try:
            ...
            try:
                # 优先使用实时计算
                from tradingagents.dataflows.realtimemetrics import get_pe_pb_with_fallback
                from tradingagents.config.databasemanager import get_database_manager
...
def _add_financial_cache_methods():
    ...
    def _get_cached_raw_financial_data(self, symbol: str) -> dict:
        """从数据库缓存获取原始财务数据"""
        try:
            from .cache.appadapter import get_mongodb_client
