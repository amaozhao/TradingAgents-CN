...
⚠️ DEPRECATED: 此模块已废弃，将在 2026-03-31 后移除
   请使用新的配置系统: app.services.configservice.ConfigService
"""
...
# 运行时设置：读取系统时区
from tradingagents.config.runtimesettings import get_timezone_name
logger = get_logger('agents')

# 导入数据模型（避免循环导入）
from .usagemodels import UsageRecord, ModelConfig, PricingConfig

try:
    from .mongodbstorage import MongoDBStorage
...
def _get_project_config_dir():
    ...
    project_root = current_file.resolve().parents[3]  # backend/tradingagents/config -> 仓库根目录
