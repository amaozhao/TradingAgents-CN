...
- 优先尝试从后端 app.services.configprovider 读取动态 system_settings（若可用）
...
def _get_system_settings_sync() -> dict:
    ...
    try:
        from app.services.configprovider import provider as config_provider  # type: ignore

        if _get_event_loop_running():
            _logger.debug("导入后检测到事件循环，跳过动态配置获取")
            return {}

        settings = asyncio.run(config_provider.get_effective_system_settings())
        return settings or {}
