...
from app.services.analysisservice import get_analysis_service
from app.services.simpleanalysisservice import get_simple_analysis_service
...
async def _get_analysis_task_for_read(task_id: str) -> Optional[Dict[str, Any]]:
    if settings.POSTGRES_READ_ENABLED:
        try:
            from app.db.analysisrepository import get_analysis_task_by_task_id
...
async def _get_analysis_report_by_task_id_for_read(task_id: str) -> Optional[Dict[str, Any]]:
    if settings.POSTGRES_READ_ENABLED:
        try:
            from app.db.analysisrepository import get_analysis_report_by_task_id
            from app.db.session import get_session_factory
...
async def _get_analysis_report_by_analysis_id_for_read(analysis_id: str) -> Optional[Dict[str, Any]]:
    if settings.POSTGRES_READ_ENABLED:
        try:
            from app.db.analysisrepository import get_analysis_report_by_analysis_id
            from app.db.session import get_session_factory
...
async def mark_task_as_failed(
    ...
    try:
        ...
        # 更新内存中的任务状态
        from app.services.memorystatemanager import TaskStatus
