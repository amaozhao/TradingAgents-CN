...
def _mask_value(key: str, value: Any) -> Any:
    ...
    if key in {"POSTGRES_URL", "REDIS_URL"} and isinstance(value, str):
        v = value
        # postgresql+asyncpg://user:pass@host:port/db?...
        v = re.sub(r"(postgresql(?:\\+asyncpg)?://[^:/?#]+):([^@/]+)@", r"\1:***@", v)
...
def _build_summary() -> Dict[str, Any]:
    ...
    # Attach derived URLs
    raw["POSTGRES_URL"] = settings.postgres_url
    ...
    try:
        ...
        try:
            ...
            get_env_api_key_for_provider = getattr(
                importlib.import_module("app.utils.keys"),
            ...
            if system_config and system_config.data_source_configs:
                logger.info(
                    f"🔍 获取到 {len(system_config.data_source_configs)} 个数据源配置"
                )
                ...
                for ds_config in system_config.data_source_configs:
                    ...
                    validation_item = {
                        ...
                        "postgres_configured": False,  # 新增：PostgreSQL 是否配置
                    ...
                    if ds_config.type in ["akshare", "yahoo"]:
                        ...
                        validation_item["postgres_configured"] = True
                    ...
                    postgres_validation["data_source_configs"].append(validation_item)

        except Exception as e:
            logger.error(f"验证 PostgreSQL 配置失败: {e}", exc_info=True)
            postgres_validation["warnings"].append(f"PostgreSQL 配置验证失败: {str(e)}")

        # 合并验证结果
        logger.info(
            f"🔍 PostgreSQL 验证结果: {len(postgres_validation['llm_providers'])} 个大模型厂家, {len(postgres_validation['data_source_configs'])} 个数据源, {len(postgres_validation['warnings'])} 个警告"
        )

        # 🔥 修改：只有必需配置有问题时才认为验证失败
        # PostgreSQL 配置警告（推荐配置）不影响总体验证结果
        ...
        return {
            ...
            "data": {
                ...
                "env_validation": {
                    ...
                    "invalid_configs": [
                        {"key": config.key, "error": error}
                        for config, error in env_result.invalid_configs
                    ],
