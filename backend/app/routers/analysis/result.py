...
if TYPE_CHECKING:
    ...
    from .setup import (
        _get_analysis_report_by_analysis_id_for_read,
...
async def get_task_result(  # pyright: ignore[reportGeneralTypeIssues]
    ...
    try:
        ...
        task_status = await analysis_service.get_task_status(task_id, user_id=current_user_id)
        ...
        if not result_data:
            ...
            logger.info(f"📊 [RESULT] 内存中未找到，尝试从 PostgreSQL document store 查找: {task_id}")
            ...
            if not postgres_result:
                ...
                tasks_doc_for_id = await _get_analysis_task_for_read(task_id, user_id=current_user_id)
                ...
                if analysis_id:
                    ...
                    postgres_result = await _get_analysis_report_by_analysis_id_for_read(
                        analysis_id, user_id=current_user_id
                    )

            if postgres_result:
                logger.info(f"✅ [RESULT] 从 PostgreSQL document store 找到结果: {task_id}")
                ...
                # 添加调试信息
                logger.info(f"📊 [RESULT] PostgreSQL数据结构: {list(result_data.keys())}")
                logger.info(f"📊 [RESULT] PostgreSQL summary长度: {len(result_data['summary'])}")
                logger.info(f"📊 [RESULT] PostgreSQL recommendation长度: {len(result_data['recommendation'])}")
                logger.info(f"📊 [RESULT] PostgreSQL decision字段: {bool(result_data.get('decision'))}")
                if result_data.get("decision"):
                    decision = result_data["decision"]
                    logger.info(
                ...
                tasks_doc = await _get_analysis_task_for_read(task_id, user_id=current_user_id)
        ...
        if "reports" not in result_data or not result_data["reports"]:
            ...
            try:
                ...
                # 2) 兼容其他保存路径
                if stock_symbol and analysis_date:
                    candidate_dirs.append(project_root / "data" / "analysis" / stock_symbol / analysis_date / "reports")
                    candidate_dirs.append(
                        project_root / "data" / "analysis" / "detailed" / stock_symbol / analysis_date / "reports"
                    )
