...
from app.worker.baostock.init import BaoStockInitService
...
async def get_database_status():
    ...
    try:
        ...
        return {"success": True, "data": status, "message": "数据库状态获取成功"}
...
async def test_baostock_connection():
    ...
    try:
        BaoStockSyncService = getattr(
            importlib.import_module("app.worker.baostock.sync"), "BaoStockSyncService"
        )
        ...
        return {
            "success": connected,
            "data": {"connected": connected, "test_time": datetime.now().isoformat()},
...
async def start_full_initialization(
    request: InitializationRequest, background_tasks: BackgroundTasks
):
    """启动完整初始化"""
    ...
    if _initialization_status["is_running"]:
        raise HTTPException(
            status_code=409, detail="初始化任务正在运行中，请等待完成后再试"
        )
...
async def start_basic_initialization(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return InitializationResponse(
            ...
            message="基础初始化任务已启动",
            task_id=task_id,
            data={"estimated_duration": "10-20分钟"},
        )
    ...
    try:
        ...
        if status["start_time"]:
            if status["is_running"]:
                ...
                duration = (
                    (status["last_update"] - status["start_time"]).total_seconds()
        ...
        return {"success": True, "data": status, "message": "状态获取成功"}
...
async def stop_initialization():
    ...
    try:
        # 更新状态
        _initialization_status.update(
            {"is_running": False, "current_task": None, "last_update": datetime.now()}
        )
...
async def _run_full_initialization_task(
    historical_days: int, force: bool, task_id: str
):
    """运行完整初始化任务"""
    global _initialization_status
    ...
    try:
        ...
        stats = await service.full_initialization(
            historical_days=historical_days, force=force
        )

        # 更新状态
        _initialization_status.update(
            {"is_running": False, "stats": stats, "last_update": datetime.now()}
        )
        ...
        _initialization_status.update(
            {"is_running": False, "last_update": datetime.now()}
        )
