...
async def create_backup(
    request: BackupRequest, current_user: dict = Depends(get_current_user)
):
    """创建数据库备份"""
    try:
...
async def cleanup_analysis(
    ...
    try:
        ...
        result = await database_service.cleanup_analysis(days)
        return {
            "success": True,
...
async def cleanup_operations(
    days: int = 90, current_user: dict = Depends(get_current_user)
):
    """清理操作日志"""
    try:
        logger.info(f"🧹 用户 {current_user['username']} 清理 {days} 天前的操作日志")
        result = await database_service.cleanup_operations(days)
        return {
            "success": True,
