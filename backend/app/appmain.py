...
from app.core.coredatabase import init_db, close_db
from app.core.loggingconfig import setup_logging
from app.routers import authdb as auth, analysisrouter as analysis, screeningrouter as screening, queue, sse, health, favorites, configrouter as config, reports, databaserouter as database, operationlogs as operation_logs, tags, tushareinitrouter as tushare_init, akshareinitrouter as akshare_init, baostockinitrouter as baostock_init, historicaldata as historical_data, multiperiodsync as multi_period_sync, financialdata as financial_data, newsdata as news_data, socialmedia as social_media, internalmessages as internal_messages, usagestatistics as usage_statistics, modelcapabilitiesrouter as model_capabilities, cache, logs
from app.routers import sync as sync_router, multisourcesync as multi_source_sync
from app.routers import stocks as stocks_router
from app.routers import stockdata as stock_data_router
from app.routers import stocksync as stock_sync_router
from app.routers import multimarketstocks as multi_market_stocks_router
from app.routers import notifications as notifications_router
from app.routers import websocketnotifications as websocket_notifications_router
from app.routers import scheduler as scheduler_router
from app.services.basicssyncservice import get_basics_sync_service
from app.services.multisourcebasicssyncservice import MultiSourceBasicsSyncService
from app.services.schedulerservice import set_scheduler_instance
# 港股和美股改为按需获取+缓存模式，不再需要定时同步任务
# from app.worker.hksyncservice import ...
# from app.worker.ussyncservice import ...
from app.middleware.operationlogmiddleware import OperationLogMiddleware
from apscheduler.schedulers.asyncio import AsyncIOScheduler
from apscheduler.triggers.cron import CronTrigger
from apscheduler.triggers.interval import IntervalTrigger
from app.services.quotesingestionservice import QuotesIngestionService
...
async def run_tushare_basic_info_sync(*args, **kwargs):
    from app.worker.tusharesyncservice import run_tushare_basic_info_sync as _run

    return await _run(*args, **kwargs)


async def run_tushare_quotes_sync(*args, **kwargs):
    from app.worker.tusharesyncservice import run_tushare_quotes_sync as _run

    return await _run(*args, **kwargs)


async def run_tushare_historical_sync(*args, **kwargs):
    from app.worker.tusharesyncservice import run_tushare_historical_sync as _run

    return await _run(*args, **kwargs)


async def run_tushare_financial_sync(*args, **kwargs):
    from app.worker.tusharesyncservice import run_tushare_financial_sync as _run

    return await _run(*args, **kwargs)


async def run_tushare_status_check(*args, **kwargs):
    from app.worker.tusharesyncservice import run_tushare_status_check as _run

    return await _run(*args, **kwargs)


async def run_akshare_basic_info_sync(*args, **kwargs):
    from app.worker.aksharesyncservice import run_akshare_basic_info_sync as _run

    return await _run(*args, **kwargs)


async def run_akshare_quotes_sync(*args, **kwargs):
    from app.worker.aksharesyncservice import run_akshare_quotes_sync as _run

    return await _run(*args, **kwargs)


async def run_akshare_historical_sync(*args, **kwargs):
    from app.worker.aksharesyncservice import run_akshare_historical_sync as _run

    return await _run(*args, **kwargs)


async def run_akshare_financial_sync(*args, **kwargs):
    from app.worker.aksharesyncservice import run_akshare_financial_sync as _run

    return await _run(*args, **kwargs)


async def run_akshare_status_check(*args, **kwargs):
    from app.worker.aksharesyncservice import run_akshare_status_check as _run

    return await _run(*args, **kwargs)


async def run_baostock_basic_info_sync(*args, **kwargs):
    from app.worker.baostocksyncservice import run_baostock_basic_info_sync as _run

    return await _run(*args, **kwargs)


async def run_baostock_daily_quotes_sync(*args, **kwargs):
    from app.worker.baostocksyncservice import run_baostock_daily_quotes_sync as _run

    return await _run(*args, **kwargs)


async def run_baostock_historical_sync(*args, **kwargs):
    from app.worker.baostocksyncservice import run_baostock_historical_sync as _run

    return await _run(*args, **kwargs)


async def run_baostock_status_check(*args, **kwargs):
    from app.worker.baostocksyncservice import run_baostock_status_check as _run
...
async def _print_config_summary(logger):
    ...
    try:
        ...
        # 检查大模型配置
        try:
            from app.services.configservice import config_service
...
async def lifespan(app: FastAPI):
    ...
    # 启动时初始化
    setup_logging()
    logger = logging.getLogger("app.appmain")

    # 验证启动配置
    try:
        from app.core.startupvalidator import validate_startup_config
    ...
    try:
        from app.core.configbridge import bridge_config_to_env
    ...
    try:
        from app.services.configprovider import provider as config_provider  # local import to avoid early DB init issues
        ...
        try:
            from app.middleware.operationlogmiddleware import set_operation_log_enabled
...
# 新增：系统配置只读摘要
from app.routers import systemconfig as system_config_router
