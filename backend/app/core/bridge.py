...
def bridge_config_to_env():
    ...
    try:
        ...
        getattr(importlib.import_module("app.services.config"), "config_service")
            ...
            if ds_config.enabled and ds_config.api_key:
                ...
                if ds_config.type.value == "tushare":
                    ...
                    # 降级到 .env 文件配置
                        ...
                        logger.info(
                            "  ℹ️  数据库中未配置有效的 TUSHARE_TOKEN，使用 .env 降级方案"
                        )
                    else:
                        logger.warning(
                            "  ⚠️  TUSHARE_TOKEN 在数据库和 .env 中都未配置有效值"
                        )
                        continue
                    bridged_count += 1
                ...
                # 🔥 优先级：数据库配置 > .env 文件
                    ...
                    # 降级到 .env 文件配置
                        ...
                        logger.info(
                            "  ℹ️  数据库中未配置有效的 FINNHUB_API_KEY，使用 .env 降级方案"
                        )
                    else:
                        logger.warning(
                            "  ⚠️  FINNHUB_API_KEY 在数据库和 .env 中都未配置有效值"
                        )
                        continue
                    bridged_count += 1
        ...
        # 6. 重新初始化 trading_agents 库的 PostgreSQL token 存储
        ...
        try:
            ...
            # 直接创建兼容存储实例，而不是调用 _init_postgres_storage()
            # 这样可以捕获更详细的错误信息
            if use_postgres.lower() == "true":
                try:
                    ...
                    if config_manager.postgres_storage.is_connected():
                        logger.info("✅ trading_agents PostgreSQL token 存储已启用")
                    else:
                        logger.warning(
                            "⚠️ trading_agents PostgreSQL token 存储连接失败，将使用 JSON 文件存储"
                        )
                        config_manager.postgres_storage = None
                except Exception as e:
                    logger.error(f"❌ 创建 PostgreSQL token 存储实例失败: {e}")
                    traceback = importlib.import_module("traceback")
                    logger.error(traceback.format_exc())
                    config_manager.postgres_storage = None
            else:
                logger.info("ℹ️ USE_POSTGRES_STORAGE 未启用，将使用 JSON 文件存储")
        except Exception as e:
