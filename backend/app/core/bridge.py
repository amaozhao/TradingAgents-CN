...
def bridge_config_to_env():
    ...
    try:
        ...
        try:
            ...
            mongodb_db = os.getenv("MONGODB_DATABASE_NAME", "trading_agents_cn")
            ...
            if use_mongodb.lower() == "true":
                try:
                    ...
                    if config_manager.mongodb_storage.is_connected():
                        logger.info("✅ trading_agents MongoDB 存储已启用")
                    else:
                        logger.warning("⚠️ trading_agents MongoDB 连接失败，将使用 JSON 文件存储")
