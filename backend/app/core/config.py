...
class Settings(BaseSettings):
    ...
    MONGODB_DATABASE: str = Field(default="trading_agents_cn")
    ...
    POSTGRES_DB: str = Field(default="trading_agents_cn")
    ...
    model_config = SettingsConfigDict(env_file=str(_PROJECT_ENV_FILE), env_file_encoding="utf-8", extra="ignore")


_COMPAT_PROPERTY_ALIASES = {
    "POSTGRES_URL": "postgres_url",
    "MONGO_URI": "mongo_uri",
    "MONGO_DB": "mongo_db",
    "MONGO_DB_IDENTITY": "mongo_db_identity",
    "REDIS_URL": "redis_url",
}
for _legacy_name, _snake_name in _COMPAT_PROPERTY_ALIASES.items():
    setattr(Settings, _legacy_name, property(lambda self, name=_snake_name: getattr(self, name)))
...
def _read_major_version() -> str:
    v = os.getenv("TRADING_AGENTS_VERSION", "").strip() or os.getenv("APP_VERSION", "").strip()
    if not v:
        try:
...
def _default_instance_tag() -> str:
    ...
    host = os.getenv("TRADING_AGENTS_DB_HOST", "").strip() or os.getenv("COMPUTERNAME", "").strip() or os.getenv("HOSTNAME", "").strip()
