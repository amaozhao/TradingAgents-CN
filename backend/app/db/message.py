...
def build_internal_messages_select(params) -> Select:
    ...
    sort_column = INTERNAL_SORT_COLUMNS.get(
        params.sort_by, InternalMessageDocument.created_time
    )
...
def build_social_media_messages_select(params) -> Select:
    ...
    sort_column = SOCIAL_SORT_COLUMNS.get(
        params.sort_by, SocialMediaMessageDocument.publish_time
    )
    statement = statement.order_by(
        desc(sort_column) if params.sort_order == -1 else asc(sort_column)
    )
...
def build_internal_message_stats_totals_select(
    *, symbol=None, start_time=None, end_time=None
    ...
    return select(
        ...
        func.coalesce(func.avg(InternalMessageDocument.confidence_level), 0).label(
            "avg_confidence"
        ),
    ).where(
        *_internal_stats_filters(
...
def build_social_media_stats_totals_select(
    *, symbol=None, start_time=None, end_time=None
    ...
    return select(
        ...
        func.coalesce(
            func.sum(
                case((SocialMediaMessageDocument.sentiment == "positive", 1), else_=0)
            ),
            0,
        ).label("positive_count"),
        func.coalesce(
            func.sum(
                case((SocialMediaMessageDocument.sentiment == "negative", 1), else_=0)
            ),
            0,
        ).label("negative_count"),
        func.coalesce(
            func.sum(
                case((SocialMediaMessageDocument.sentiment == "neutral", 1), else_=0)
            ),
            0,
        ).label("neutral_count"),
        func.coalesce(func.sum(cast(engagement["views"].astext, Integer)), 0).label(
            "total_views"
        ),
        func.coalesce(func.sum(cast(engagement["likes"].astext, Integer)), 0).label(
            "total_likes"
        ),
        func.coalesce(func.sum(cast(engagement["shares"].astext, Integer)), 0).label(
            "total_shares"
        ),
        func.coalesce(func.sum(cast(engagement["comments"].astext, Integer)), 0).label(
            "total_comments"
        ),
        func.coalesce(func.avg(SocialMediaMessageDocument.engagement_rate), 0).label(
            "avg_engagement_rate"
        ),
    ).where(*filters)
...
async def get_internal_message_stats(
    session, *, symbol=None, start_time=None, end_time=None
) -> dict[str, Any]:
    ...
    filters = _internal_stats_filters(
        symbol=symbol, start_time=start_time, end_time=end_time
    )
    return {
        "total_count": int(totals.total_count or 0),
        "avg_confidence": float(totals.avg_confidence or 0),
        "message_types": await _count_by(
            session, InternalMessageDocument.message_type, filters
        ),
        "categories": await _count_by(
            session, InternalMessageDocument.category, filters
        ),
        "departments": await _count_by(
            session, InternalMessageDocument.department, filters
        ),
        "importance_levels": await _count_by(
            session, InternalMessageDocument.importance, filters
        ),
...
def _internal_filters(params) -> list[Any]:
    ...
    if params.author:
        filters.append(
            InternalMessageDocument.payload["source"]["author"].astext == params.author
        )
    ...
    if params.min_confidence:
        filters.append(
            InternalMessageDocument.confidence_level >= params.min_confidence
        )
    if params.rating:
        filters.append(InternalMessageDocument.rating == params.rating)
    if params.keywords:
        filters.append(
            _jsonb_array_contains_any(
                InternalMessageDocument.payload["keywords"], params.keywords
            )
        )
    if params.tags:
        filters.append(
            _jsonb_array_contains_any(
                InternalMessageDocument.payload["tags"], params.tags
            )
        )
    return filters
...
def _social_filters(params) -> list[Any]:
    ...
    if params.min_influence_score:
        filters.append(
            SocialMediaMessageDocument.influence_score >= params.min_influence_score
        )
    if params.min_engagement_rate:
        filters.append(
            SocialMediaMessageDocument.engagement_rate >= params.min_engagement_rate
        )
    ...
    if params.keywords:
        filters.append(
            _jsonb_array_contains_any(
                SocialMediaMessageDocument.payload["keywords"], params.keywords
            )
        )
    if params.hashtags:
        filters.append(
            _jsonb_array_contains_any(
                SocialMediaMessageDocument.payload["hashtags"], params.hashtags
            )
        )
    return filters


def _internal_stats_filters(
    *, symbol=None, start_time=None, end_time=None
) -> list[Any]:
...
async def _count_by(session, column, filters: list[Any]) -> dict[str, int]:
    result = await session.execute(
        select(column, func.count().label("count")).where(*filters).group_by(column)
    )
