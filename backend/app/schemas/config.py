...
class LLMProvider(BaseModel):
    ...
    api_secret: Optional[str] = Field(
        default=None, description="API密钥（某些厂家需要）"
    )
    ...
    # 🆕 聚合渠道支持
    is_aggregator: bool = Field(
        default=False, description="是否为聚合渠道（如302.AI、OpenRouter）"
    )
    aggregator_type: Optional[str] = Field(
        default=None, description="聚合渠道类型（openai_compatible/custom）"
    )
    model_name_format: Optional[str] = Field(
        default=None, description="模型名称格式（如：{provider}/{model}）"
    )
...
class ModelInfo(BaseModel):
    ...
    input_price_per_1k: Optional[float] = Field(
        default=None, description="输入价格(每1K tokens)"
    )
    output_price_per_1k: Optional[float] = Field(
        default=None, description="输出价格(每1K tokens)"
    )
    ...
    capabilities: List[str] = Field(
        default_factory=list, description="能力标签(如: vision, function_calling)"
    )

    # 🆕 聚合渠道模型映射支持
    original_provider: Optional[str] = Field(
        default=None, description="原厂商标识（用于聚合渠道）"
    )
    original_model: Optional[str] = Field(
        default=None, description="原厂商模型名（用于能力映射）"
    )
...
class DataSourceType(str, Enum):
    ...
    注意：这个枚举与 trader.constants.DataSourceCode 保持同步
    添加新数据源时，请先在 trader/constants/data_sources.py 中注册
    """
...
class LLMConfig(BaseModel):
    ...
    api_key: Optional[str] = Field(
        default=None, description="API密钥(可选，优先从厂家配置获取)"
    )
    ...
    model_category: Optional[str] = Field(
        default=None, description="模型类别(用于OpenRouter等)"
    )
    ...
    # 定价配置
    input_price_per_1k: Optional[float] = Field(
        default=None, description="输入token价格(每1000个token)"
    )
    output_price_per_1k: Optional[float] = Field(
        default=None, description="输出token价格(每1000个token)"
    )
...
class DataSourceConfig(BaseModel):
    ...
    description: Optional[str] = Field(default=None, description="配置描述")
    # 新增字段：支持市场分类
    market_categories: Optional[List[str]] = Field(
        default_factory=list, description="所属市场分类列表"
    )
    ...
    created_at: Optional[datetime] = Field(
        default_factory=now_tz, description="创建时间"
    )
    updated_at: Optional[datetime] = Field(
        default_factory=now_tz, description="更新时间"
    )
...
class UsageStatistics(BaseModel):
    ...
    total_cost: float = Field(
        default=0.0, description="总成本（已废弃，使用 cost_by_currency）"
    )
    cost_by_currency: Dict[str, float] = Field(
        default_factory=dict, description="按货币统计的成本"
    )
    by_provider: Dict[str, Any] = Field(
        default_factory=dict, description="按供应商统计"
    )
...
class SystemConfig(BaseModel):
    ...
    llm_configs: List[LLMConfig] = Field(
        default_factory=list, description="大模型配置列表"
    )
    default_llm: Optional[str] = Field(default=None, description="默认大模型")

    # 数据源配置
    data_source_configs: List[DataSourceConfig] = Field(
        default_factory=list, description="数据源配置列表"
    )
    default_data_source: Optional[str] = Field(default=None, description="默认数据源")

    # 数据库配置
    database_configs: List[DatabaseConfig] = Field(
        default_factory=list, description="数据库配置列表"
    )

    # 系统设置
    system_settings: Dict[str, Any] = Field(
        default_factory=dict, description="系统设置"
    )

    # 元数据
    created_at: datetime = Field(default_factory=now_tz)
    updated_at: datetime = Field(default_factory=now_tz)
    created_by: Optional[PyDocumentId] = Field(default=None, description="创建者")
    updated_by: Optional[PyDocumentId] = Field(default=None, description="更新者")
