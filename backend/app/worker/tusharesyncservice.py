...
from app.services.appstockdataservice import get_stock_data_service
from app.services.historicaldataservice import get_historical_data_service
from app.services.newsdataservice import get_news_data_service
from app.core.coredatabase import get_mongo_db
from app.core.coreconfig import settings
from app.core.ratelimiter import get_tushare_rate_limiter
...
class TushareSyncService:
            ...
            for i, symbol in enumerate(symbols):
                try:
                    ...
                    if (i + 1) % 20 == 0:
                        ...
                        # 更新任务进度
                        if job_id:
                            from app.services.schedulerservice import update_job_progress, TaskCancelledException
                            try:
    ...
    async def _save_financial_data(self, symbol: str, financial_data: Dict[str, Any]) -> bool:
        ...
        try:
            # 使用统一的财务数据服务
            from app.services.financialdataservice import get_financial_data_service
    ...
    async def _update_progress(self, job_id: str, progress: int, message: str):
        ...
        """
        try:
            from app.services.schedulerservice import TaskCancelledException
