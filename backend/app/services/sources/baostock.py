...
class BaoStockAdapter(DataSourceAdapter):
    ...
    def get_stock_list(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            try:
                ...
                df["ts_code"] = code_series.str.replace("sh.", "").str.replace(
                    "sz.", ""
                ) + code_series.str.extract(r"^(sh|sz)\.").iloc[
                    :, 0
                ].str.upper().str.replace("SH", ".SH").str.replace("SZ", ".SZ")
                ...
                if industry_rs.error_code == "0":
                    ...
                    if industry_list:
                        industry_df = pd.DataFrame(
                            industry_list, columns=industry_rs.fields
                        )
                        ...
                        industry_map = dict(
                            zip(industry_df["code"], industry_df["industry_clean"])
                        )
                        # 将行业信息合并到主DataFrame
    ...
    def get_daily_basic(
        self, trade_date: str, max_stocks: Optional[int] = None
            ...
            try:
                ...
                total_stocks = len(
                    [
                        s
                        for s in stock_list
                        if len(s) > 5 and s[4] == "1" and s[5] == "1"
                    ]
                )
                logger.info(
                    f"📊 BaoStock: 找到 {total_stocks} 只活跃股票，开始处理{'全部' if max_stocks is None else f'前 {max_stocks} 只'}..."
                )
                    ...
                    if stock_type == "1" and status == "1":
                        try:
                            ...
                            if rs_valuation.error_code == "0":
                                ...
                                if valuation_data:
                                    ...
                                    if processed_count % 50 == 0:
                                        ...
                                        logger.info(
                                            f"📈 BaoStock 同步进度: {processed_count}/{total_stocks} ({progress_pct:.1f}%) - 最新: {name}({ts_code})"
                                        )
                                else:
                                    failed_count += 1
                            else:
                            ...
                            if failed_count % 50 == 0:
                                logger.warning(
                                    f"⚠️ BaoStock: 已有 {failed_count} 只股票获取失败"
                                )
                            logger.debug(
                                f"BaoStock: Failed to get valuation for {code}: {e}"
                            )
                            continue
                if basic_data:
                    df = pd.DataFrame(basic_data)
                    logger.info(
                        f"✅ BaoStock 同步完成: 成功 {len(df)} 只，失败 {failed_count} 只，日期 {trade_date}"
                    )
                    return df
                else:
                    logger.warning(
