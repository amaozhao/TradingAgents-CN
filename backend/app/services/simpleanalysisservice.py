...
from app.models.analysismodels import (
...
from app.services.memorystatemanager import get_memory_state_manager, TaskStatus
from app.services.redisprogresstracker import RedisProgressTracker, get_progress_by_id
from app.services.progressloghandler import register_analysis_tracker, unregister_analysis_tracker
...
def _ensure_tradingagents_logging() -> None:
    ...
    from tradingagents.utils.logginginit import init_logging
...
def _get_stock_info_safe(stock_code: str):
    ...
    try:
        if _data_source_manager is None:
            from tradingagents.dataflows.datasourcemanager import get_data_source_manager
...
# 设置日志
logger = logging.getLogger("app.services.simpleanalysisservice")
        ...
        if doc and "llm_configs" in doc:
            ...
            for config_dict in llm_configs:
                if config_dict.get("model_name") == model_name:
                    ...
                    from tradingagents.llmclients.providerkeys import normalize_provider_key, default_backend_url
...
def _get_env_api_key_for_provider(provider: str) -> str:
    ...
    from tradingagents.llmclients.providerkeys import env_key_for_provider, normalize_provider_key
...
def _get_default_backend_url(provider: str) -> str:
    ...
    Returns:
        str: 默认的 backend_url
    """
    from tradingagents.llmclients.providerkeys import default_backend_url, normalize_provider_key
...
def create_analysis_config(
    ...
    from tradingagents.defaultconfig import DEFAULT_CONFIG
...
class SimpleAnalysisService:
    ...
    def __init__(self):
        ...
        # 设置 WebSocket 管理器
        try:
            from app.services.websocketmanager import get_websocket_manager
    ...
    async def execute_analysis_background(
        ...
        try:
            ...
            from tradingagents.utils.stockvalidator import prepare_stock_data_async
            from datetime import datetime
    ...
    def _run_analysis_sync(
        ...
        try:
            ...
            # 🆕 智能模型选择逻辑
            from app.services.modelcapabilityservice import get_model_capability_service
    ...
    async def _list_user_tasks_from_postgres(
        ...
        *,
        ...
        if not settings.POSTGRES_READ_ENABLED:
            return None
        try:
            from app.db.analysisrepository import list_user_analysis_tasks
            ...
            try:
                if market_info.get("market") == "china_a":
                    ...
                    if stock_info and "股票名称:" in stock_info:
                        ...
                        try:
                            from tradingagents.dataflows.datasourcemanager import get_china_stock_info_unified as get_info_dict
                    ...
                    # 港股：使用改进的港股工具
                    try:
                        from tradingagents.dataflows.providers.hk.improvedhk import get_hk_company_name_improved
