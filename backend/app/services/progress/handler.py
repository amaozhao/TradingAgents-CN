...
logger = logging.getLogger("app.services.progress.log")
            ...
            for task_id, tracker in trackers_copy.items():
                try:
                    ...
                    if (
                        ...
                        and tracker.progress_data.get("status") == "running"
                    ):
                        tracker.update_progress(progress_message)
                        logger.debug(
                            f"📊 [进度日志] 更新进度: {task_id} -> {progress_message}"
                        )
                        break  # 只更新第一个匹配的跟踪器
...
def get_logs() -> ProgressLogHandler:
    """获取全局进度日志处理器实例"""
    global _logs

    with _handler_lock:
        if _logs is None:
            _logs = ProgressLogHandler()
            ...
            for logger_name in loggers_to_monitor:
                ...
                target_logger.addHandler(_logs)
                target_logger.setLevel(logging.INFO)

            logger.info(
                f"📊 [进度日志] 已注册到 {len(loggers_to_monitor)} 个日志记录器"
            )
