...
# 统一指标库
from trader.tools.analysis.indicators import IndicatorSpec, compute_many
...
class ScreeningService:
        ...
        for code in symbols:
            try:
                ...
                if need_base:
                    ...
                    # 仅基本面条件：使用基本面快照判断
                    get_cn_fund_snapshot = getattr(
                        importlib.import_module(
                            "trader.flows.providers.china.fundamentals"
                        ),
                        "get_cn_fund_snapshot",
                    )
    ...
    def _get_universe(self) -> List[str]:
        """获取A股代码集合：从 PostgreSQL stock_basic_info 集合获取所有A股股票代码"""
        try:
