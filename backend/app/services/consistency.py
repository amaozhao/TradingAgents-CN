...
class DataConsistencyChecker:
    ...
    def _get_stock_metric_value(
        self, df: pd.DataFrame, stock_code: str, metric: str
    ) -> Optional[float]:
        """获取特定股票的指标值"""
        try:
            # 尝试不同的匹配方式
    ...
    def _calculate_overall_consistency(
        ...
        is_consistent = (
            significant_differences <= len(comparisons) * 0.3
        )  # 允许30%的指标有显著差异
    ...
    def resolve_data_conflicts(
        ...
        if action == "use_either":
            ...
            return (
                primary_data,
                f"数据存在轻微差异（置信度: {consistency_result.confidence_score:.2f}），使用主数据源",
            )
            ...
            return (
                primary_data,
                f"数据差异显著（置信度: {consistency_result.confidence_score:.2f}），仅使用主数据源",
            )
