...
def build_research_prompt(
    *, principal: ResearchPrincipal, tools: list[ResearchTool]
    ...
    return (
        ...
        "Do not call analyze_trade_journal unless the user supplied journal text, "
        "artifact_id, or file_id in this session. Do not call run_shadow_backtest unless "
        "the request or prior tool results provide returns or trades[].pnl. If these "
        "owner-scoped inputs are missing, ask the user to upload or paste the missing data "
        "instead of calling the tool.\n"
        "Call trading_check before trading_account, trading_positions, trading_orders, "
        "trading_history, or trading_quote. Do not call trading_account, trading_positions, "
        ...
        "stock_analysis runs the Agent single-stock DAG-parity workflow; it replicates "
        "the original TradingAgents DAG nodes, tools, debates, risk review, and report schema. "
        "Do not submit the old DAG queue for new Agent single-stock requests. "
        "single_stock_analysis is a compatibility alias for the same DAG-parity workflow "
        "and writes a report compatible with the existing report pages. If the user asks for an "
        "existing task, use stock_analysis_status and stock_analysis_report.\n"
