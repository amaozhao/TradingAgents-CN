...
from app.services.simpleanalysisservice import create_analysis_config, get_provider_by_model_name
...
from app.core.redisclient import get_redis_service, RedisKeys
from app.services.queueservice import QueueService
from app.core.coredatabase import get_redis_client
from app.services.redisprogresstracker import RedisProgressTracker
...
class AnalysisService:
    ...
    def _execute_analysis_sync_with_progress(self, task: AnalysisTask, progress_tracker: RedisProgressTracker) -> AnalysisResult:
        ...
        try:
            ...
            # 使用标准配置函数创建完整配置
            from app.core.unifiedconfig import unified_config
            ...
            # 使用标准配置函数创建完整配置
            from app.services.simpleanalysisservice import create_analysis_config
    ...
    async def _execute_single_analysis_async(self, task: AnalysisTask):
        ...
        try:
            ...
            try:
                ...
                # 根据模型名称确定供应商
                from app.services.simpleanalysisservice import get_provider_by_model_name
    ...
    async def _update_task_status(
        ...
        """更新任务状态（委托至拆分的工具函数）"""
        try:
            from app.services.analysis.statusupdateutils import perform_update_task_status
    ...
    async def _update_task_status_with_tracker(
        ...
        try:
            from app.services.analysis.statusupdateutils import perform_update_task_status_with_tracker
