...
def demo_service_status():
    ...
    for key, value in status.items():
        if key == "service_available":
            ...
            logger.info(f"  {icon} PostgreSQL状态: {value}")
    ...
    for stock_code in stock_codes:
        ...
        if API_AVAILABLE:
            ...
            if db_manager.is_postgres_available():
                try:
                    collection = db_manager.postgres_db["stock_basic_info"]
...
def demo_fallback_mechanism():
    ...
    logger.info("🔄 降级机制说明:")
    logger.info("  1. 优先从PostgreSQL获取数据")
    logger.info("  2. PostgreSQL不可用时，降级到Tushare数据接口")
    logger.info("  3. Tushare数据接口不可用时，提供基础的降级数据")
    logger.info("  4. 获取到的数据会自动缓存到PostgreSQL（如果可用）")
...
def main():
    ...
    try:
        ...
        logger.info("\n💡 使用建议:")
        logger.info("  1. 确保PostgreSQL已正确配置以获得最佳性能")
