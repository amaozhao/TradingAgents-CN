...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            os.path.join("data", "logs", "akshare_init.log"), encoding="utf-8"
        ),
    ],
)
    ...
    try:
        ...
        if result.get("errors"):
            ...
            for error in result["errors"][:3]:  # 只显示前3个错误
                print(
                    f"     - {error.get('step', 'Unknown')}: {error.get('error', 'Unknown error')}"
                )
...
async def main():
    ...
    parser.add_argument(
        "--test-connection", action="store_true", help="测试AKShare连接"
    )
