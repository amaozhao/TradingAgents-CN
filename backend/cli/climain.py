...
# 项目内部导入
from cli.climodels import AnalystType
from cli.cliutils import (
    ask_output_language,
    ensure_api_key,
...
def setup_cli_logging():
    ...
    """
    import logging
    from tradingagents.utils.loggingmanager import get_logger_manager
...
def check_api_keys(llm_provider: str) -> bool:
    ...
    if missing_keys:
        ...
        logger.info(f"\n3. 运行 'python -m cli.climain config' 查看详细配置说明")
...
def run_analysis():
    ...
    with Live(layout, refresh_per_second=DEFAULT_REFRESH_RATE) as live:
        ...
        try:
            from tradingagents.utils.stockvalidator import prepare_stock_data
...
def help_chinese():
    ...
    logger.info(f"1. [cyan]python -m cli.climain config[/cyan]     # 查看配置信息")
    logger.info(f"2. [cyan]python -m cli.climain examples[/cyan]   # 查看示例程序")
    logger.info(f"3. [cyan]python -m cli.climain test[/cyan]       # 运行测试")
    logger.info(f"4. [cyan]python -m cli.climain analyze[/cyan]    # 开始股票分析")
            ...
            if e.code == 2 and len(sys.argv) > 1:
                ...
                if suggestions:
                    ...
                    for suggestion in suggestions:
                        logger.info(f"   • [cyan]python -m cli.climain {suggestion}[/cyan]")
                    logger.info(f"\n[dim]使用 [cyan]python -m cli.climain help[/cyan] 查看所有可用命令[/dim]")
                else:
                    logger.error(f"\n[red]❌ 未知命令: '{unknown_command}'[/red]")
                    logger.info(f"[yellow]使用 [cyan]python -m cli.climain help[/cyan] 查看所有可用命令[/yellow]")
            raise e
