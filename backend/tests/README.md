...
# 阿里百炼集成测试
python backend/tests/integration/test_dashscope_integration.py

# Tushare数据接口测试
python backend/tests/fast_tdx_test.py
python backend/tests/test_tdx_integration.py

# Redis性能测试
python backend/tests/quick_redis_test.py
python backend/tests/test_redis_performance.py
```

### 诊断工具
```bash
# 诊断Gemini模型问题
python backend/tests/diagnose_gemini_25.py

# 检查导入问题
python backend/tests/debug_imports.py

# 检查所有可用的Gemini模型
python backend/tests/check_gemini_models.py
```
...
# 智能系统测试
python backend/tests/test_smart_system.py

# 降级系统演示
python backend/tests/demo_fallback_system.py
```
