...
from backend.scripts.postgres_rollback_check import check_rollback_evidence, write_rollback_target_manifest
...
def test_rollback_check_writes_non_secret_target_manifest(tmp_path):
    ...
    output_path = write_rollback_target_manifest(
        ...
        env={
            ...
            "TRADINGAGENTS_API_TOKEN": "secret-token",
        },
    )

    payload = json.loads(output_path.read_text(encoding="utf-8"))
    assert output_path == tmp_path / "00_target_manifest.json"
    assert payload["target_env"] == "tradingagents-test"
...
def test_rollback_manifest_requires_target_environment(tmp_path):
    ...
    with pytest.raises(ValueError, match="provide --target-env or TRADINGAGENTS_TARGET_ENV"):
...
def test_rollback_check_cli_reports_missing_target_environment_as_json(tmp_path, monkeypatch, capsys):
    ...
    assert "provide --target-env or TRADINGAGENTS_TARGET_ENV" in payload["checks"][0]["detail"]
