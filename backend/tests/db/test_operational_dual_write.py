...
from app.models.operation_log import ActionType, OperationLogCreate
from app.services import operation_log_service
from app.services.database import backups
...
async def test_delete_database_backup_dual_writes_tombstone(monkeypatch):
    ...
    monkeypatch.setattr(backups, "get_mongo_db", lambda: db)
    monkeypatch.setattr(backups, "dual_write_hot_document", fake_dual_write)
    monkeypatch.setattr(backups.os.path, "exists", lambda _path: False)

    await backups.delete_backup(str(backup_id))
...
async def test_import_supported_collection_dual_writes_tombstones_and_inserted_docs(monkeypatch):
    ...
    monkeypatch.setattr(backups, "get_mongo_db", lambda: db)
    monkeypatch.setattr(backups, "dual_write_hot_documents", fake_dual_write_many)

    result = await backups.import_data(
