...
from scripts.postgres_cutover_gate import build_gate_steps, run_gate
...
def test_build_gate_steps_can_include_runtime_log_check(tmp_path):
    ...
    assert "backend/scripts/postgres_runtime_log_check.py" in runtime_step.command
...
def test_cutover_gate_dry_run_writes_summary_without_running_commands(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TRADINGAGENTS_API_BASE_URL", "https://example.invalid")
    ...
    assert summary["environment"]["TRADINGAGENTS_API_TOKEN_SET"] is True
...
def test_cutover_gate_writes_non_secret_target_manifest_when_target_is_named(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TRADINGAGENTS_EXPECT_POSTGRES_READ_ENABLED", "true")
    monkeypatch.setenv("TRADINGAGENTS_EXPECT_POSTGRES_DUAL_WRITE_ENABLED", "true")
...
def test_cutover_gate_require_explicit_env_fails_fast_when_target_env_missing(tmp_path, monkeypatch):
    for name in [
        ...
        "TRADINGAGENTS_API_TOKEN",
        "TRADINGAGENTS_API_USERNAME",
        "TRADINGAGENTS_API_PASSWORD",
        "TRADINGAGENTS_EXPECT_POSTGRES_READ_ENABLED",
        "TRADINGAGENTS_EXPECT_POSTGRES_DUAL_WRITE_ENABLED",
        "TRADINGAGENTS_TARGET_ENV",
        "TRADINGAGENTS_CUTOVER_PHASE",
    ]:
    ...
    assert "TRADINGAGENTS_API_BASE_URL" in summary["results"][0]["detail"]
    assert "TRADINGAGENTS_EXPECT_POSTGRES_READ_ENABLED" in summary["results"][0]["detail"]
    assert "TRADINGAGENTS_TARGET_ENV or --target-env" in summary["results"][0]["detail"]
    assert "TRADINGAGENTS_CUTOVER_PHASE or --target-phase" in summary["results"][0]["detail"]
...
def test_cutover_gate_does_not_write_incomplete_manifest_when_preflight_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("TRADINGAGENTS_TARGET_ENV", raising=False)
...
def test_cutover_gate_require_explicit_env_passes_and_runs_gates_with_target_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MONGODB_DATABASE", "tradingagentscn")
    monkeypatch.setenv("POSTGRES_HOST", "postgres.example.internal")
    monkeypatch.setenv("POSTGRES_DB", "tradingagentscn")
    monkeypatch.setenv("POSTGRES_USER", "postgres")
    monkeypatch.setenv("POSTGRES_PASSWORD", "secret-postgres")
    monkeypatch.setenv("TRADINGAGENTS_API_BASE_URL", "https://api.example.internal")
    monkeypatch.setenv("TRADINGAGENTS_API_TOKEN", "secret-token")
    monkeypatch.setenv("TRADINGAGENTS_EXPECT_POSTGRES_READ_ENABLED", "true")
    monkeypatch.setenv("TRADINGAGENTS_EXPECT_POSTGRES_DUAL_WRITE_ENABLED", "true")
