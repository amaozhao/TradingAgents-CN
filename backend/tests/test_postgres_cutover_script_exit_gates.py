...
from scripts import postgres_consistency_check, postgres_migration_inventory


def test_consistency_check_exits_nonzero_when_report_is_inconsistent(monkeypatch, capsys):
...
def test_consistency_check_allow_inconsistent_keeps_zero_exit(monkeypatch, capsys):
    ...
    monkeypatch.setattr(postgres_consistency_check, "_run_cli", fake_run_cli)
    monkeypatch.setattr("sys.argv", ["postgres_consistency_check.py", "--sample-limit", "25", "--allow-inconsistent"])
...
def test_inventory_exits_nonzero_on_contract_regression(tmp_path, monkeypatch, capsys):
    def fake_scan_backend(root, output):
        ...
        return {"summary": {"response_model_dict_endpoints": 1, "raw_dict_request_bodies": 0}}

    monkeypatch.setattr(postgres_migration_inventory, "scan_backend", fake_scan_backend)
    monkeypatch.setattr("sys.argv", ["postgres_migration_inventory.py", "--root", str(tmp_path)])
...
def test_inventory_allow_contract_regressions_keeps_zero_exit(tmp_path, monkeypatch, capsys):
    def fake_scan_backend(root, output):
        assert root == str(tmp_path)
        return {"summary": {"response_model_dict_endpoints": 1, "raw_dict_request_bodies": 1}}

    monkeypatch.setattr(postgres_migration_inventory, "scan_backend", fake_scan_backend)
    monkeypatch.setattr(
        "sys.argv",
        ["postgres_migration_inventory.py", "--root", str(tmp_path), "--allow-contract-regressions"],
    )
