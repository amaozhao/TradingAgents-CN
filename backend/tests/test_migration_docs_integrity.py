...
def test_completion_audit_covers_every_task_plan_item():
    task_plan = (MIGRATION_DOCS / "postgres_migration_task_plan.md").read_text(encoding="utf-8")
    completion_audit = (MIGRATION_DOCS / "postgres_completion_audit.md").read_text(encoding="utf-8")

    planned_tasks = _sort_task_ids(set(re.findall(r"^## (T\d+)\.", task_plan, flags=re.MULTILINE)))
    audited_tasks = _sort_task_ids(set(re.findall(r"^### (T\d+)\.", completion_audit, flags=re.MULTILINE)))
...
def test_inventory_named_write_collections_have_migration_decisions():
    ...
    docs_text = "\n".join(path.read_text(encoding="utf-8") for path in MIGRATION_DOCS.glob("*.md"))
    named_collections = {
        item["collection"]
        for item in inventory["mongo_writes"]
...
def test_unresolved_dynamic_mongo_writes_have_manual_audit_rows():
    inventory = _load_inventory()
    long_tail_doc = (MIGRATION_DOCS / "postgres_long_tail_coverage.md").read_text(encoding="utf-8")
    unresolved_writes = [
        item
        for item in inventory["mongo_writes"]
...
def test_operator_script_mongo_writes_have_manual_audit_rows():
    inventory = _load_inventory()
    long_tail_doc = (MIGRATION_DOCS / "postgres_long_tail_coverage.md").read_text(encoding="utf-8")
    script_writes = [
        item
...
def test_cutover_runbook_keeps_api_smoke_as_post_read_acceptance():
    runbook = (MIGRATION_DOCS / "postgres_cutover_runbook.md").read_text(encoding="utf-8")
    stop_conditions = _section(runbook, "## Stop Conditions")
    post_read = _section(runbook, "## Post-Read Acceptance Conditions")

    assert "Do not move from `dual_write` to `postgres_read_mongo_fallback`" in stop_conditions
    assert "API smoke is missing" not in stop_conditions
    assert "API smoke is missing or `all_passed=false`" in post_read
    assert "after entering `postgres_read_mongo_fallback`" in runbook


def test_worker_dual_write_coverage_matches_inventory():
    inventory = _load_inventory()
    coverage_doc = (MIGRATION_DOCS / "worker_dual_write_coverage.md").read_text(encoding="utf-8")
    documented_rows = _parse_worker_coverage_rows(coverage_doc)
    worker_write_files = set(inventory["worker_mongo_write_files"])
    inventory_rows = {
        (
            item["path"],
            item["line"],
...
def _load_inventory() -> dict:
    return json.loads((MIGRATION_DOCS / "postgres_inventory.json").read_text(encoding="utf-8"))
