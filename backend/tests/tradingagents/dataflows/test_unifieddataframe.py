...
from tradingagents.dataflows.unifieddataframe import get_china_daily_df_unified


def test_unified_dataframe_prefers_tushare_then_akshare_then_baostock():
    # 模拟三个来源：tushare成功，后两个不应被调用
    with mock.patch('tradingagents.dataflows.unifieddataframe.get_tushare_adapter') as m_ts, \
         mock.patch('tradingagents.dataflows.unifieddataframe.get_akshare_provider') as m_ak, \
         mock.patch('tradingagents.dataflows.unifieddataframe.get_baostock_provider') as m_bs, \
         mock.patch('tradingagents.dataflows.unifieddataframe.get_data_source_manager') as m_dsm:
