# Backend Dockerfile for FastAPI service (TradingAgents-CN v1.0.1)
# 前后端分离架构 - 后端服务
# 支持多架构: amd64, arm64

# 使用 Python 3.13，与 backend/pyproject.toml 的最低运行版本保持一致
...
# 复制后端源码和打包元数据（pyproject 位于 backend/）
...
RUN pip install --upgrade pip -i https://pypi.tuna.tsinghua.edu.cn/simple && \
    pip install --prefer-binary ./backend -i https://pypi.tuna.tsinghua.edu.cn/simple && \
